...
test("inventory position subtracts reservations without moving physical stock", () => {
  assert.deepEqual(inventoryPosition({
    on_hand_qty_micros: 100 * Q,
    inbound_qty_micros: 20 * Q,
    outbound_qty_micros: 15 * Q,
    reserved_qty_micros: 30 * Q,
  }), {
    on_hand_qty_micros: 100 * Q,
    inbound_qty_micros: 20 * Q,
    outbound_qty_micros: 15 * Q,
    reserved_qty_micros: 30 * Q,
    projected_qty_micros: 75 * Q,
  });
});

test("min-max replenishment triggers below min and replenishes exactly to max", () => {
  const plan = planMinMaxReplenishment({
    on_hand_qty_micros: 20 * Q,
    inbound_qty_micros: 5 * Q,
    reserved_qty_micros: 12 * Q,
  }, {
    min_qty_micros: 15 * Q,
    max_qty_micros: 40 * Q,
    safety_stock_qty_micros: 10 * Q,
  });
...
test("safety stock remains diagnostic and invalid min-max policy fails closed", () => {
  const plan = planMinMaxReplenishment({ on_hand_qty_micros: 8 * Q }, {
    min_qty_micros: 5 * Q,
    max_qty_micros: 20 * Q,
    safety_stock_qty_micros: 10 * Q,
  });
  ...
  assert.throws(() => planMinMaxReplenishment({ on_hand_qty_micros: Q }, {
    min_qty_micros: 20 * Q,
    max_qty_micros: 10 * Q,
  }), /max_qty_micros/);
});
