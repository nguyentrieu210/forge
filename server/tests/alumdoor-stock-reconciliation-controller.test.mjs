...
function context({ action = "save", document, existing = existingData(), actor } = {}) {
  ...
  return {
    ...
    value: {
      ...
      existing: {
        ...
        modified_at: "2026-08-02T09:05:00.000Z",
        data: existing,
        children: [],
      },
      nextVersion: 2,
...
test("reconciliation save uses historical snapshot for surplus row and creates no stock ledger", async () => {
  ...
  const { value, sourceReader } = context({ document, existing });
  const plan = await controller.buildPlan(value);

  assert.equal(plan.document.docstatus, 0);
  assert.equal(plan.document.data.items[0].book_qty_micros, 10_000_000, "existing captured book snapshot must stay immutable on save");
  assert.equal(plan.document.data.items[1].book_qty_micros, 3_000_000, "new bulk row must snapshot through the canonical reader");
  assert.equal(plan.document.data.items[1].variance_qty_micros, 0);
  assert.equal(plan.stock_entries.length, 0, "saving a draft must never post stock ledger");
...
test("reconciliation submit remains four-eyes and is the only path that posts variance", async () => {
  ...
  const document = {
    ...existing,
    items: [{ ...existing.items[0], counted_qty: 9, variance_reason: "Sai số đếm" }],
  };
  ...
  assert.equal(plan.stock_entries.length, 1, "submit must post the variance through the canonical ledger path");
