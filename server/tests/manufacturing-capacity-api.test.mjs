...
const ENDPOINT = "https://tenant.test/api/method/metaforge.manufacturing.preview_capacity_plan";
...
function request(body){return new Request(ENDPOINT,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});}
...
test("capacity API returns finite schedule and subtracts visible downtime",async()=>{const ctx=context();const response=await routeManufacturingCapacityApi(request({production_plan:"PLAN-1",through_date:"2026-08-05"}),new URL(ENDPOINT),ctx.value);assert.equal(response.status,200);const payload=await response.json();assert.equal(payload.message.operations[0].required_minutes,"120.000000");assert.equal(payload.message.workstation_summary[0].downtime_minutes,"60.000000");assert.equal(ctx.permissionCalls.length,4);});

test("capacity API fails closed when relevant routing is hidden",async()=>{const ctx=context({hidden:new Set(["RT-FG"])});await assert.rejects(()=>routeManufacturingCapacityApi(request({production_plan:"PLAN-1",through_date:"2026-08-05"}),new URL(ENDPOINT),ctx.value),/relevant Manufacturing Routing is outside/);});

test("capacity API fails closed when relevant downtime is hidden",async()=>{const ctx=context({hidden:new Set(["DT-1"])});await assert.rejects(()=>routeManufacturingCapacityApi(request({production_plan:"PLAN-1",through_date:"2026-08-05"}),new URL(ENDPOINT),ctx.value),/Relevant workstation downtime is outside/);});

test("capacity API rejects client tenant selection",async()=>{const ctx=context();await assert.rejects(()=>routeManufacturingCapacityApi(request({production_plan:"PLAN-1",through_date:"2026-08-05",tenant_id:"other"}),new URL(ENDPOINT),ctx.value),/tenant scope is controlled/);});
