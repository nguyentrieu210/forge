...
function purchaseOrderData(qtyBar, transactionDate, rowId = "ROW-1") {
  ...
  const qty = theoreticalKg.toFixed(3);
  return {
    supplier: "FACTORY-1",
    company: "Alumdoor",
    currency: "VND",
    ...
    items: [{
      ...
      uom: "Kg",
      stock_uom: "Cây",
      conversion_factor: (qtyBar / Number(qty)).toFixed(6),
      rate: 100_000,
    }],
  };
}
...
test("rollout disabled keeps the legacy Purchase Order path", async () => {
  ...
  const draft = await apply(poController, store, "create", "PO-LEGACY", purchaseOrderData(10, "2026-06-30"), "PO-LEGACY-create");
...
test("enabled rollout opens obligations and Receipt 230 allocates FIFO as 200 + 30", async () => {
  ...
  const receiptDraft = await apply(receiptController, store, "create", "PR-01", receiptData([receiptItem(230, "ROW-1", 630)]), "PR-01-create");
  await apply(receiptController, store, "submit", "PR-01", receiptDraft.data, "PR-01-submit");

  const snapshot = store.snapshot();
  assert.deepEqual(snapshot.purchase_allocation_entries.map((entry) => ({ purchase_order: entry.purchase_order, qty_micros: entry.qty_micros })), [
  ...
  const remaining = await store.listPurchaseAllocationObligations(tenant, snapshot.purchase_allocation_entries[0].queue_key, snapshot.purchase_allocation_entries[0].window_id);
  assert.deepEqual(remaining.map((entry) => ({ purchase_order: entry.purchase_order, remaining_qty_micros: entry.remaining_qty_micros })), [
...
test("a later PO consumes unapplied Receipt quantity with source voucher and weight conservation", async () => {
  ...
  const receiptDraft = await apply(receiptController, store, "create", "PR-EXCESS", receiptData([receiptItem(105, "PR-ROW-1", 300)]), "PR-EXCESS-create");
  ...
  const applied = snapshot.purchase_allocation_entries.find((entry) => entry.entry_kind === "apply_unapplied" && entry.purchase_order === "PO-B");
  const movement = snapshot.purchase_unapplied_entries.find((entry) => entry.entry_kind === "apply" && entry.allocation_entry_id === applied?.entry_id);
  assert.ok(applied);
  assert.ok(movement);
  ...
  const compatibility = snapshot.procurement_entries.find((entry) => entry.purchase_order === "PO-B" && entry.qty_micros === 3_000_000);
  ...
  assert.equal(cancelled.purchase_unapplied_entries.reduce((sum, entry) => sum + (entry.projected_actual_weight_micros ?? 0), 0), 0);
});
...
test("multiple Receipt lines create ordered unapplied sources consumed once by a later PO", async () => {
  ...
  const receiptDraft = await apply(receiptController, store, "create", "PR-MULTI", receiptData([
  ...
  snapshot = store.snapshot();
  const applied = snapshot.purchase_allocation_entries.filter((entry) => entry.entry_kind === "apply_unapplied" && entry.purchase_order === "PO-MULTI-2");
  ...
  const remainingSources = await store.listPurchaseUnappliedQueueSources(tenant, receives[0].queue_key, receives[0].window_id);
