...
test("first-party App Factory metadata packs through the canonical manifest parser", async () => {
  ...
  assert.equal(definition.fields.find((field) => field.fieldname === "definition_status")?.fieldtype, "Select");
  assert.equal(definition.fields.some((field) => field.fieldname === "status"), false, "system status must not be redeclared as app metadata");
