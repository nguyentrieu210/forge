...
function createPlatform({ secondCompany = "ALUMDOOR" } = {}) {
  const order1 = {
    ...
    docstatus: 1,
    items: [aluminiumLine("AL71", 7.2, 200)],
  };
  const order2 = {
    name: "PO-DAY-2",
    supplier: "Tiến Đạt",
    ...
    docstatus: 1,
    items: [aluminiumLine("AL71", 7.2, 100)],
  };
  ...
  const platform = {
    async fetch(outbound) {
      ...
      if (path === "/resource/Purchase Receipt" && outbound.method === "POST") {
        ...
        creates += 1;
        const doc = { name: `PR-BULK-${creates}`, docstatus: 0, ...body };
  ...
  return new Request("https://app.local/api/method/alumdoor.purchase.bulk_fifo_receipt", {
    ...
    body: JSON.stringify({
      args: {
        supplier: "Tiến Đạt",
        warehouse: "KHO-1",
        supplier_invoice_no: "TD-GIAO-001",
        driver: "QA",
        lines,
        ...overrides,
      },
...
test("bulk preview cộng dồn các dòng cùng quy cách nên không ăn lại đơn cũ", async () => {
  ...
  assert.equal(body.line_count, 2);
  assert.equal(body.total_qty_bar, 230);
...
test("bulk commit tạo đúng một Purchase Receipt nháp chứa toàn bộ dòng và retry idempotent", async () => {
  const state = createPlatform();
  const payload = [line(200, 560.16), line(30, 84.024)];
  const first = await handleBulkPurchaseFifoRequest(request(payload), { PLATFORM: state.platform }, true);
  const firstBody = await first.json();
  assert.equal(first.status, 200, firstBody.message);
  assert.equal(firstBody.purchase_receipt, "PR-BULK-1");
  assert.equal(firstBody.draft, true);
  assert.equal(firstBody.replayed, false);
  assert.equal(state.creates, 1);
  const stored = state.drafts.get("PR-BULK-1");
  assert.equal(stored.items.length, 2);
  assert.deepEqual(stored.items.map((row) => [row.purchase_order, row.qty_bar]), [
    ["PO-DAY-1", 200],
    ["PO-DAY-2", 30],
  ]);
  assert.match(stored.note, /^\[bulk-fifo:[0-9a-f]{64}\]/);

  const retry = await handleBulkPurchaseFifoRequest(request(payload), { PLATFORM: state.platform }, true);
  const retryBody = await retry.json();
  assert.equal(retry.status, 200, retryBody.message);
  assert.equal(retryBody.purchase_receipt, "PR-BULK-1");
  assert.equal(retryBody.replayed, true);
  assert.equal(state.creates, 1, "exact retry must not create another draft");
});

test("cùng số phiếu giao nhưng payload khác bị chặn để tránh duplicate operation", async () => {
  const state = createPlatform();
  const first = await handleBulkPurchaseFifoRequest(request([line(10, 28.008)]), { PLATFORM: state.platform }, true);
  assert.equal(first.status, 200);
  const conflict = await handleBulkPurchaseFifoRequest(request([line(11, 30.8088)]), { PLATFORM: state.platform }, true);
  const body = await conflict.json();
  assert.equal(conflict.status, 422);
  assert.match(body.message, /đã gắn với PR-BULK-1|không tạo trùng/i);
  assert.equal(state.creates, 1);
});

test("bulk fail closed khi các dòng phân bổ qua đơn mua khác công ty", async () => {
  const state = createPlatform({ secondCompany: "OTHER-CO" });
  const result = await handleBulkPurchaseFifoRequest(request([
    line(200, 560.16),
    line(30, 84.024),
  ]), { PLATFORM: state.platform }, false);
  ...
  assert.match(body.message, /nhiều Công ty/i);
  assert.equal(state.creates, 0);
});

test("bulk bắt tenant platform call, supplier delivery note và giới hạn 100 dòng", async () => {
  const state = createPlatform();
  const missingInvoice = await handleBulkPurchaseFifoRequest(request([line(1, 2.8008)], { supplier_invoice_no: "" }), { PLATFORM: state.platform }, false);
  assert.equal(missingInvoice.status, 422);
  assert.match((await missingInvoice.json()).message, /Số phiếu giao NCC/);

  const tooMany = await handleBulkPurchaseFifoRequest(request(Array.from({ length: 101 }, () => line(1, 2.8008))), { PLATFORM: state.platform }, false);
  ...
  const noTenant = new Request("https://app.local/api/method/alumdoor.purchase.preview_bulk_fifo_receipt", {
    method: "POST",
    headers: { "content-type": "application/json", "x-cloudforge-callback": "https://gateway.local/api" },
    body: JSON.stringify({ args: { supplier: "Tiến Đạt", warehouse: "KHO-1", supplier_invoice_no: "X", lines: [line(1, 2.8008)] } }),
  });
  const denied = await handleBulkPurchaseFifoRequest(noTenant, { PLATFORM: state.platform }, false);
