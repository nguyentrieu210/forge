...
const context = (overrides = {}) => ({
  currency: "VND", currencyScale: 0, baseSalaryMinor: 20_000_000, grossEarningsMinor: 30_000_000,
  preRuleDeductionsMinor: 0, workingDays: 22, paymentHalfUnits: 44,
  statutoryInputs: { dependents: 2, insured: true }, ...overrides,
});
...
test("statutory evaluator calculates progressive marginal tiers deterministically", () => {
  const result = evaluatePayrollRuleFormula(formula({
    ...
    pit: { op: "progressive", value: { output: "taxable" }, tiers: [{ up_to: "5000000", rate_bps: 500 }, { up_to: "10000000", rate_bps: 1000 }, { up_to: null, rate_bps: 1500 }] },
  }), context());
...
test("legacy audit-only rule remains compatible when statutory inputs are unused", () => {
  const result = evaluatePayrollRuleFormula("{}", context({ statutoryInputs: {} }));
  assert.equal(result.schemaVersion, 0);
  assert.equal(result.canonicalFormulaJson, "{}");
  assert.deepEqual(result.outputs, {});
  assert.throws(() => evaluatePayrollRuleFormula("{}", context({ statutoryInputs: { dependents: 2 } })), /Legacy payroll rule cannot consume statutory inputs/);
});
...
test("salary structure rejects mapping the same payroll rule output twice", async () => {
  ...
  const masters = { "Company:Demo": { default_currency: "VND" }, "Currency:VND": { currency_scale: 0 }, "VN Payroll Rule:RULE-DUP": { rule_code: "RULE-DUP", effective_from: "2026-01-01", legal_document_no: "LEGAL", source_url: "https://example.test/legal", formula_json: formulaJson, approved_by: "payroll@example.test", approved_at: "2026-01-01T00:00:00Z" }, "Account:Payroll Payable": {}, "Account:Tax Payable": {}, "Account:Other Payable": {}, "Cost Center:CC-1": {}, "Salary Component:Tax": { type: "Deduction", account: "Tax Payable" }, "Salary Component:Other Tax": { type: "Deduction", account: "Other Payable" } };
  const documents = { "Holiday List:HL-1": { name: "HL-1", docstatus: 1, version: 1, data: { company: "Demo" } } };
  const reader = { async getDocument(_tenant, doctype, name) { return documents[`${doctype}:${name}`] ?? null; }, async getMasterRecordData(_tenant, doctype, name) { return masters[`${doctype}:${name}`] ?? null; }, async listDocumentsByDoctype() { return []; }, async hasMasterRecord(_tenant, doctype, name) { return Boolean(masters[`${doctype}:${name}`]); }, async getPeriodLockDate() { return null; } };
  const ctx = { command: { tenant_id: "demo", aggregate: { doctype: "Salary Structure", name: "SS-DUP" }, action: "submit", actor: { user_id: "payroll@example.test", roles: ["Payroll Manager"] }, document: { structure_name: "SS-DUP", company: "Demo", currency: "VND", effective_from: "2026-01-01", payroll_rule: "RULE-DUP", payroll_payable_account: "Payroll Payable", default_cost_center: "CC-1", holiday_list: "HL-1", unmarked_attendance: "Vắng", components: [{ salary_component: "Tax", amount_type: "Payroll Rule Output", rule_output_key: "tax" }, { salary_component: "Other Tax", amount_type: "Payroll Rule Output", rule_output_key: "tax" }] } }, reader, existing: null, nextVersion: 1, now: "2026-08-02T00:00:00Z" };
