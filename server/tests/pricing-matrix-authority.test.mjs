...
function authority(seed = [], options = {}) {
  ...
  const mutationCalls = [];
  let failOnceMatching = options.failOnceMatching ?? "";
  ...
  const context = {
    ...
    mutations: {
      async create(input) {
        ...
        if (failOnceMatching && input.idempotencyKey.includes(failOnceMatching)) {
          failOnceMatching = "";
...
test("retry after an explicit partial failure continues safely without duplicating already-applied effects", async () => {
  const { context, rows, mutationCalls } = authority(baseSeed(), { failOnceMatching: ":price:" });
  const payload = {
  ...
  assert.equal(mutationCalls.filter((id) => id === "req-retry:item").length, 1);
});
...
test("replaying the same desired price is a no-op even with the original OCC token", async () => {
  ...
  assert.equal(mutationCalls.filter((id) => id.includes("req-idempotent-update")).length, 1);
});
