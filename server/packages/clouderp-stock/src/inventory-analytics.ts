...
export interface AbcInput {
  key: string;
  annual_consumption_value_minor: number;
}

export interface AbcResult extends AbcInput {
  cumulative_value_minor: number;
  cumulative_ratio: number;
  class: "A" | "B" | "C";
}

export interface MovementAgeInput {
  key: string;
  last_movement_at?: string;
  on_hand_qty_micros: number;
}

export interface MovementAgeResult extends MovementAgeInput {
  age_days: number | null;
  status: "No Stock" | "Active" | "Slow" | "Dead" | "Never Moved";
}
...
/** Age remaining positive stock layers into caller-defined inclusive day buckets. */
export function ageInventory(
  layers: AgingLayer[],
  asOf: string,
  bucketUpperDays: number[],
): AgingBucket[] {
  const asOfTime = timestamp(asOf, "as_of");
  const uppers = [...bucketUpperDays];
  let previous = -1;
  for (const [index, upper] of uppers.entries()) {
    if (!Number.isSafeInteger(upper) || upper < 0 || upper <= previous) {
      throw errors.validation(`bucketUpperDays[${index}] must be an increasing non-negative integer`);
    }
    previous = upper;
  }
  const buckets: AgingBucket[] = uppers.map((upper, index) => ({
    min_age_days: index === 0 ? 0 : uppers[index - 1]! + 1,
    max_age_days: upper,
    qty_micros: 0,
    value_minor: 0,
  }));
  buckets.push({
    min_age_days: uppers.length ? uppers[uppers.length - 1]! + 1 : 0,
    qty_micros: 0,
    value_minor: 0,
  });
  ...
  for (const [index, layer] of layers.entries()) {
    ...
    if (!Number.isSafeInteger(bucket.qty_micros) || !Number.isSafeInteger(bucket.value_minor)) {
      throw errors.validation("Inventory aging total exceeds safe integer bounds");
    }
  }
  return buckets;
}
...
/**
 * Classifies items by cumulative annual consumption value. Cutoffs are explicit policy
 * inputs, e.g. 0.8/0.95, rather than hidden ERP constants.
 */
export function classifyAbc(inputs: AbcInput[], aCutoff: number, bCutoff: number): AbcResult[] {
  if (!(aCutoff > 0 && aCutoff < bCutoff && bCutoff < 1)) {
    throw errors.validation("ABC cutoffs must satisfy 0 < A < B < 1");
  }
  ...
  return rows.map((row) => {
    ...
    return {
      ...row,
      cumulative_value_minor: cumulative,
      cumulative_ratio: ratio,
      class: ratio <= aCutoff ? "A" : ratio <= bCutoff ? "B" : "C",
    };
  });
}

/** Slow/dead classification with explicit thresholds and no mutation side effect. */
export function classifyMovementAge(
  inputs: MovementAgeInput[],
  asOf: string,
  slowAfterDays: number,
  deadAfterDays: number,
): MovementAgeResult[] {
  if (!Number.isSafeInteger(slowAfterDays) || !Number.isSafeInteger(deadAfterDays)
    || slowAfterDays < 0 || deadAfterDays <= slowAfterDays) {
    throw errors.validation("Movement thresholds must satisfy 0 <= slow < dead");
  }
  const asOfTime = timestamp(asOf, "as_of");
