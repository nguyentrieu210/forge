...
export async function ingestMarketplaceProviderRecord(
...
): Promise<MarketplaceOperationalOrderResult & { channel_profile: string; warehouse: string }> {
  ...
  return { channel_profile: resolved.channel_profile, warehouse: resolved.warehouse, ...result };
}
