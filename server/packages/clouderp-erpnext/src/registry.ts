...
import { CutOrderController, StockReconciliationController, StockReservationController } from "./alumdoor-inventory.js";
...
import { WarehouseCashCountController, WarehouseCashFundController, WarehouseCashTransferController, WarehouseCashVoucherController } from "./warehouse-cash.js";

export function registerErpNextCoreControllers(registry: ControllerRegistry): ControllerRegistry {
  return registry
    .register(new CreditNoteController()).register(new DebitNoteController()).register(new StockReturnController())
    .register(new VersionedBillOfMaterialsController()).register(new StockUomSnapshotWorkOrderController()).register(new RolloutManufacturingStockEntryController())
    ...
    .register(new AdditionalSalaryController()).register(new EmployeeBenefitEnrollmentController()).register(new EmployeeLoanController()).register(new EmployeeLoanRepaymentController()).register(new EmployeeAdvanceController()).register(new TravelRequestController()).register(new EmployeeFinalSettlementController())
    .register(new ExtendedGoalController()).register(new HrmAppraisalController()).register(new Review360Controller()).register(new CompetencyController()).register(new CompetencyAssessmentController()).register(new TalentPoolController()).register(new SuccessionPlanController()).register(new TrainingCourseController()).register(new TrainingEventController()).register(new TrainingAssessmentController()).register(new EmployeeCertificateController())
    .register(new PosOpeningEntryController()).register(new PosInvoiceController()).register(new PosClosingEntryController()).register(new BankTransactionController()).register(new BankReconciliationController())
    .register(new HrmSalarySlipController()).register(new PayrollEntryController()).register(new SalaryBankBatchController()).register(new SubscriptionController()).register(new EInvoiceSubmissionController())
    .register(new WarehouseCashFundController()).register(new WarehouseCashVoucherController()).register(new WarehouseCashTransferController()).register(new WarehouseCashCountController())
    .register(new CutOrderController()).register(new StockReservationController()).register(new StockReconciliationController());
}
