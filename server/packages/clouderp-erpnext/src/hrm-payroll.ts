...
export async function buildHrmSalarySlipInputs(
  ...
  if (!assignmentName) throw errors.validation("Salary Slip requires earnings or salary_structure_assignment");
  ...
  const holidayListName = requiredText(assignment.data.holiday_list ?? structure.data.holiday_list, "Salary Structure holiday_list");
  ...
  for (const workDate of workDates) {
    ...
    if (status === "Có mặt" || status === "Làm việc từ xa") paymentUnits += 2;
    else if (status === "Nửa ngày") paymentUnits += 1;
  ...
  if (!Array.isArray(rawComponents) || rawComponents.length === 0) throw errors.reference(`Salary Structure ${structureName} has no components`);

  const earnings: SalarySlipComponentRow[] = [];
  const deductions: SalarySlipComponentRow[] = [];
  const ruleComponents: Array<{ index: number; componentName: string; componentType: string; outputKey: string; account: string; costCenter?: string }> = [];
  for (const [index, rawValue] of rawComponents.entries()) {
    if (!rawValue || typeof rawValue !== "object" || Array.isArray(rawValue)) throw errors.reference(`Salary Structure component row ${index + 1} is invalid`);
    ...
    const costCenter = text(raw.cost_center) || text(assignment.data.payroll_cost_center) || text(structure.data.default_cost_center);
    ...
    } else throw errors.reference(`${componentName} amount_type is invalid`);
    if (amountMinor < 0) throw errors.reference(`${componentName} amount cannot be negative`);
    if (truthy(raw.prorate_by_payment_days)) amountMinor = multiplyRatio(amountMinor, paymentUnits, workDates.length * 2);
    const row: SalarySlipComponentRow = {
      row_id: `STRUCT-${index + 1}`, salary_component: componentName,
      amount: fromScaledInt(amountMinor, currencyScale), amount_minor: amountMinor, account,
  ...
  for (const [index, additional] of additionalDocs.entries()) {
    ...
    const row: SalarySlipComponentRow = {
      row_id: `ADDL-${index + 1}-${additional.name}`, salary_component: componentName,
      amount: fromScaledInt(amountMinor, currencyScale), amount_minor: amountMinor, account,
      ...(costCenter ? { cost_center: costCenter } : {}),
    };
    (componentType === "Earning" ? earnings : deductions).push(row);
  ...
  const statutory = evaluatePayrollRuleFormula(formulaJson, {
    currency, currencyScale, baseSalaryMinor,
  ...
  for (const ruleComponent of ruleComponents) {
    ...
    if (!Number.isSafeInteger(amountMinor) || amountMinor < 0) throw errors.reference(`Payroll rule output ${ruleComponent.outputKey} must be a non-negative minor-unit integer`);
    const row: SalarySlipComponentRow = {
      row_id: `RULE-${ruleComponent.index + 1}`, salary_component: ruleComponent.componentName,
      amount: fromScaledInt(amountMinor, currencyScale), amount_minor: amountMinor, account: ruleComponent.account,
  ...
  const payrollPayableAccount = text(assignment.data.payable_account) || requiredText(structure.data.payroll_payable_account, "Salary Structure payroll payable account");
  const sourceSnapshot = {
    employee: input.employee, company: input.company, start_date: input.start_date, end_date: input.end_date,
    ...
    payroll_rule: {
      name: payrollRuleName, rule_code: ruleCode, effective_from: ruleFrom,
      ...(ruleTo ? { effective_to: ruleTo } : {}), legal_document_no: legalDocumentNo, source_url: sourceUrl,
      approved_by: approvedBy, approved_at: approvedAt, formula_schema_version: statutory.schemaVersion,
      formula_sha256: formulaHash, statutory_inputs: statutory.inputs, statutory_outputs_minor: statutory.outputs,
    },
    ...
    working_days: workDates.length, payment_days: paymentUnits / 2,
  };
  const inputHash = await sha256(JSON.stringify(sourceSnapshot));
  return {
    salary_structure_assignment: assignmentName, payroll_period: payrollPeriod.name, payroll_payable_account: payrollPayableAccount,
    earnings, deductions, working_days: workDates.length, payment_days: paymentUnits / 2,
    input_hash: inputHash, rule_trace_json: JSON.stringify(sourceSnapshot),
  };
}
...
async function requireSubmitted(context: ControllerContext<SalarySlipData>, doctype: string, name: string): Promise<CanonicalDocument<JsonObject>> {
...
function requiredText(value: unknown, field: string): string { const result = text(value); if (!result) throw errors.reference(`${field} is required`); return result; }
function date(value: unknown, field: string): string {
  const result = text(value);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(result) || Number.isNaN(Date.parse(`${result}T00:00:00Z`))) throw errors.reference(`${field} must use YYYY-MM-DD`);
  return result;
}
function optionalDate(value: unknown, field: string): string | undefined { const result = text(value); return result ? date(result, field) : undefined; }
...
function parseWeeklyOff(value: string): Set<number> {
  ...
  for (const raw of value.split(",").map((part) => part.trim()).filter(Boolean)) {
    const day = Number(raw); if (!Number.isInteger(day) || day < 0 || day > 6) throw errors.reference("Holiday List weekly_off_days must contain integers 0..6"); result.add(day);
  }
  return result;
}
function parseHolidayDates(value: string): Set<string> {
  if (!value) return new Set();
  let parsed: unknown; try { parsed = JSON.parse(value); } catch { throw errors.reference("Holiday List holidays_json must be valid JSON"); }
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(item))) throw errors.reference("Holiday List holidays_json must be an array of YYYY-MM-DD strings");
  return new Set(parsed);
}
function enumerateDates(fromDate: string, toDate: string): string[] {
  const result: string[] = [];
  for (let cursor = Date.parse(`${fromDate}T00:00:00Z`), end = Date.parse(`${toDate}T00:00:00Z`); cursor <= end; cursor += 86_400_000) result.push(new Date(cursor).toISOString().slice(0, 10));
  return result;
}
function isWorkingDay(dateValue: string, weeklyOff: Set<number>, holidays: Set<string>): boolean { return !weeklyOff.has(new Date(`${dateValue}T00:00:00Z`).getUTCDay()) && !holidays.has(dateValue); }
function safeAmountAdd(left: number, right: number): number { const result = Number(BigInt(left) + BigInt(right)); if (!Number.isSafeInteger(result)) throw errors.validation("Payroll amount exceeds safe integer bounds"); return result; }
function multiplyRatio(value: number, numerator: number, denominator: number): number {
  if (!Number.isSafeInteger(value) || !Number.isSafeInteger(numerator) || !Number.isSafeInteger(denominator) || denominator <= 0) throw errors.validation("Payroll ratio inputs exceed safe integer bounds");
  const result = Number((BigInt(value) * BigInt(numerator) + BigInt(Math.floor(denominator / 2))) / BigInt(denominator));
  if (!Number.isSafeInteger(result)) throw errors.validation("Payroll amount exceeds safe integer bounds"); return result;
}
function byName(left: { name: string }, right: { name: string }): number { return left.name.localeCompare(right.name); }
