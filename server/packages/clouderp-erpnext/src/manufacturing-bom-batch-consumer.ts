...
/**
 * Integration authority for BOM create/replay. The implementation must delegate commit to
 * the ordinary canonical BOM Draft path and retain its permission/version collision checks.
 */
export interface ManufacturingBomBatchGateway<TResult = JsonObject> {
  ...
  previewCanonicalDraft?(
    context: ManufacturingBomBatchTrustedContext,
    input: BulkBomDraftInput,
  ): Promise<TResult>;
...
/** Domain-only decode of A1 table-itemized executor value into the proven BOM bulk input. */
export function parseManufacturingBomBatchExecutorValue(
  ...
  return {
    ...
    ...(shared.operating_cost === undefined
      ? {}
      : { operating_cost: decimal(shared.operating_cost, "shared_inputs.operating_cost") }),
