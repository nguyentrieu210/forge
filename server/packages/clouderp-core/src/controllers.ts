...
async function applyBuyingPricing<T extends PurchaseItem>(context:ControllerContext<JsonObject>,items:T[],priceList:string|undefined,currency:string,postingDate:string,supplier:string,supplierGroup?:string):Promise<T[]>{if(!priceList)return items;return Promise.all(items.map(async item=>{const qtyMicros=item.qty_micros??toScaledInt(item.qty,6);const price=await resolveServerPrice(context,{itemCode:item.item_code,qtyMicros,postingDate,priceList,documentCurrency:currency,...(item.uom?{uom:item.uom}:{}),partyType:"Supplier",party:supplier,...(supplierGroup?{supplierGroup}:{})});return{...item,rate:price.rate,rate_minor:price.rate_minor,item_price:price.item_price,...(price.pricing_rule?{pricing_rule:price.pricing_rule}:{}),...(price.discount_percentage?{discount_percentage:price.discount_percentage}:{})};}));}
