...
/**
 * Compares submitted Supplier Quotations against one RFQ without inventing FX math.
 * Overall ranking uses server-computed company-currency base totals only; line rates remain
 * in the quotation currency so a VND rate is never silently compared with a USD rate.
 */
export function compareSupplierQuotations(
  ...
  const rfqRows = rfq.items.map((item, index) => ({
  ...
  for (const quote of quotations) {
    ...
    for (const [index, item] of data.items.entries()) {
      ...
      const rateMinor = integerMinor(item.rate_minor ?? toScaledInt(item.rate, scale, `${quote.name}.items[${index}].rate`), `${quote.name}.items[${index}].rate_minor`);
  ...
  const completeRank = documentRows
    ...
    .sort((a, b) => (a.base_grand_total_minor! - b.base_grand_total_minor!) || a.quotation.localeCompare(b.quotation))
...
/**
 * Records an explicit commercial decision. It deliberately does not auto-select the cheapest
 * quotation: price is evidence, not a substitute for approval, quality, lead-time or contract terms.
 */
...
/**
 * Pure 3-way-match evaluator. Callers provide authoritative PO/Receipt/Invoice facts; this
 * function emits deterministic variance/hold facts and never posts AP/GL by itself.
 */
...
function resolveQuotedRfqRow(
  item: PurchaseItem,
  rfqRows: Array<{ row_id: string; item: RequestForQuotationData["items"][number]; requested_qty_micros: number; uom: string | null }>,
  byId: Map<string, { row_id: string; item: RequestForQuotationData["items"][number]; requested_qty_micros: number; uom: string | null }>,
  quotation: string,
  index: number,
) {
...
function compareLineOffers(a: QuotationLineOffer, b: QuotationLineOffer): number {
  if (a.currency === b.currency && a.currency_scale === b.currency_scale) {
    return (a.rate_minor - b.rate_minor) || a.quotation.localeCompare(b.quotation);
  }
...
function integerScale(value: unknown, field: string, fallback: number): number {
  const scale = value === undefined ? fallback : value;
  if (!Number.isSafeInteger(scale) || Number(scale) < 0 || Number(scale) > 9) throw errors.validation(`${field} must be an integer from 0 to 9`);
  return Number(scale);
}
...
function nonNegativeInt(value: unknown, field: string): number {
  if (!Number.isSafeInteger(value) || Number(value) < 0) throw errors.validation(`${field} must be a non-negative safe integer`);
  return Number(value);
}
