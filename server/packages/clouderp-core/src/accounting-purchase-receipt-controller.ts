...
export class AccountingPurchaseReceiptController implements DocumentController<PurchaseReceiptData> {
  ...
  async buildPlan(context: ControllerContext<PurchaseReceiptData>): Promise<MutationPlan<PurchaseReceiptData>> {
    ...
    const value = addMinor(plan.stock_entries.map((line) => Math.max(0, line.stock_value_difference_minor)), "Purchase Receipt stock value");
    ...
    for (const line of plan.stock_entries) {
      ...
      gl.push(
        { line_key: `STOCK-${line.line_key}`, account: stockAccount, debit_minor: lineValue, credit_minor: 0, currency, currency_scale: scale, posting_at: plan.document.data.posting_at },
        { line_key: `SRBNB-${line.line_key}`, account: receivedNotBilled, debit_minor: 0, credit_minor: lineValue, currency, currency_scale: scale, posting_at: plan.document.data.posting_at },
      );
    }
    plan.document.data = { ...plan.document.data, stock_account: stockAccount, stock_received_but_not_billed: receivedNotBilled };
    plan.gl_entries = gl;
    return plan;
  }
}
