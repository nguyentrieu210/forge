...
import type { SemanticValueDefinition } from "./index.js";
...
function requireStableId(value: string, field: string): void {
  if (!STABLE_ID.test(value)) throw errors.validation(`${field} must be a stable lowercase id`);
}
...
function exactMetric(metric: PlanningMetricDefinition): boolean {
  return metric.value.exact === true || metric.value.scale !== undefined;
}
...
function normalizeDimensions(value: Record<string, string>, field: string): Record<string, string> {
  ...
  for (const [key, raw] of entries) {
    ...
    if (!raw.trim() || raw.length > 200) throw errors.validation(`${field}.${key} must be non-empty and at most 200 characters`);
    output[key] = raw;
  }
  return output;
}
...
export class PlanningMetricRegistry {
  ...
  constructor(definitions: PlanningMetricDefinition[]) {
    for (const definition of definitions) {
      ...
      if (definition.value.scale !== undefined && (!Number.isSafeInteger(definition.value.scale) || definition.value.scale < 1)) {
        throw errors.validation(`Planning metric ${definition.id} scale must be a positive safe integer`);
      }
...
export class ScenarioPlanningEngine {
  ...
  project(baseline: PlanningCell[], scenario: PlanningScenario): PlanningProjection {
    ...
    if (scenario.adjustments.length > 500) throw errors.validation(`Scenario ${scenario.id} has too many adjustments`);
    ...
    for (const adjustment of scenario.adjustments) {
      ...
      if (!(["set", "delta", "basis_points"] as PlanningAdjustmentOperation[]).includes(adjustment.operation)) {
        throw errors.validation(`Adjustment ${adjustment.id} has unsupported operation`);
      }
