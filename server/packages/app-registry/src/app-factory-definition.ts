...
function date(value: unknown, field: string): string {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(normalized) || Number.isNaN(Date.parse(`${normalized}T00:00:00Z`))) throw errors.validation(`${field} must be YYYY-MM-DD`);
  return normalized;
}
...
function validateDefinitionPayload(definitionKind: AppFactoryDefinitionKind, definition: JsonObject, knownFields: ReadonlySet<string>): JsonObject {
  ...
  for (const property of Object.keys(definition)) if (!allowed.has(property)) throw errors.validation(`Process definition property is not supported: ${property}`);
  return {
...
export class AppFactoryDefinitionController implements DocumentController<AppFactoryDefinitionData> {
  ...
  async buildPlan(context: ControllerContext<AppFactoryDefinitionData>): Promise<MutationPlan<AppFactoryDefinitionData>> {
    if (context.command.action === "submit" || context.command.action === "cancel") throw errors.lifecycle("App Factory Definition is lifecycle-managed, not submittable");
    ...
    const siblings = all.filter((document) => document.name !== context.command.aggregate.name && document.data.definition_key === definitionKey && document.data.definition_kind === definitionKind);
    ...
    const document: CanonicalDocument<AppFactoryDefinitionData> = {
      ...
      owner: existing?.owner ?? context.command.actor.user_id, docstatus: 0, status: nextStatus,
      version: context.nextVersion, created_at: existing?.created_at ?? context.now, modified_at: context.now,
      data, children: [],
    };
    return {
      ...
      events: [domainEvent({
        type: eventType, tenantId: context.command.tenant_id, aggregate: context.command.aggregate,
        aggregateVersion: context.nextVersion, actor: context.command.actor.user_id,
        commandId: context.command.command_id, occurredAt: context.now,
        payload: { definition_key: definitionKey, definition_kind: definitionKind, version_no: versionNo, status: nextStatus, ...(statusChanged && reason ? { reason } : {}) },
      })],
      result: {
        doctype: this.doctype, name: document.name, version: document.version,
        definition_key: definitionKey, definition_kind: definitionKind, definition_version: versionNo, status: nextStatus,
      },
    };
  }
}
