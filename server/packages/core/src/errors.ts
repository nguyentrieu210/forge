...
export function asCloudForgeError(value: unknown): CloudForgeError {
  ...
  if (message.includes("INVOICE_DUE_DATE_REQUIRED")) return errors.validation("Invoice due date is required before submission");
  if (message.includes("INVOICE_DUE_DATE_INVALID")) return errors.validation("Invoice due date must be a valid YYYY-MM-DD date");
  if (message.includes("INVOICE_DUE_DATE_BEFORE_POSTING")) return errors.validation("Invoice due date cannot be before the posting date");
  if (message.includes("INVOICE_POSTING_DATE_INVALID")) return errors.validation("Invoice posting date is invalid");
  if (message.includes("PAYMENT_ADVANCE_CONTEXT_MISMATCH")) {
    return errors.reference("Payment advance company, party, account and currency must match the source Payment Entry");
  }
