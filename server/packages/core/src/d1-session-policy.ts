...
/**
 * HTTP bookmarks are opaque D1 tokens. We only enforce transport safety here;
 * D1 remains the authority that decides whether a bookmark belongs to this DB.
 */
 ...
 *
 * - authoritative: first query must observe the primary; use for command/status
 *   decisions and all write-side coordination.
 * - replica-safe: a supplied bookmark preserves read-your-writes; without one,
 *   the first query may use any sufficiently available D1 instance.
 */
...
/**
 * Run a harmless read inside the same Session to expose D1 routing metadata.
 * Cloudflare only fills served_by_* for remote D1 requests; local workerd leaves
 * them absent, which is intentionally represented as null.
 */
