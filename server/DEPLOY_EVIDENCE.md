...
- ~~**Single hostname, therefore one tenant.**~~ **No longer true (2026-07-27.)** The
  credential can create custom domains, and the gateway now serves five of them —
  `edu`, `hrm`, `chotdon`, `alu` and `phanbon` under `kairo.vn` — each resolving to its
  own tenant through the `ROUTES` KV. The route key is the hostname, as designed.
- ~~**No R2 bucket bound.**~~ **No longer true (2026-07-27.)** `cloudforge-files` is
  created and bound as `FILES` on every tenant Worker, including ones generated by
  `provision-tenant.mjs`. Product photographs upload and serve on the live deployment.
