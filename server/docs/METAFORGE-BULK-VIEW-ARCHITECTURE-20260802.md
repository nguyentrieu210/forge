...
Bulk View không phải page riêng cho từng nghiệp vụ. Một renderer đọc metadata để quyết định:
...
Phiên bản đầu chỉ hỗ trợ `commitStrategy=document_update` và fail closed với transaction, DocType submittable, child table, single/tree và field `internal/read-only/server-owned`. Field có `read_only_depends_on` cũng không được sửa trong Bulk v1 vì grid chưa đánh giá điều kiện readonly theo từng dòng. Save dùng `modified` để giữ optimistic concurrency; lỗi được báo theo từng dòng.

## Vì sao không cho mọi DocType dùng cùng một bulk editor
...
3. **Transaction/ledger** — phải đi qua controller/action nghiệp vụ; không cập nhật stock, công nợ, trạng thái submit hay snapshot bằng mass document update.

Bulk View v1 chỉ giải quyết nhóm 1. Nhóm 2 và 3 có thể dùng UX grid nhưng commit adapter phải khác.
...
Các field cấu thành identity/name được hiển thị để nhận diện nhưng không mở sửa hàng loạt trong v1.
...
Các cấu hình như `Item × Color`, `User × Role`, `User × Warehouse`, `Supplier × Item` phù hợp Matrix View hơn danh sách row thông thường.
...
Đổi cha hàng loạt phải kiểm tra cycle/descendant; không dùng generic cell update.
...
Không cần tạo ViewKind mới. Bulk selection gọi print queue chung để:
...
Pivot, grouping, aggregation, chart và export là năng lực của Report/Analysis, không tạo thêm loại DocType view chỉ vì hình bảng khác.

## Authoring boundary hiện tại

Runtime contract canonical là `viewPolicy.bulk`. Với brief lớn như ALUM, sibling `<brief>.views.json` được loader kiểm tra riêng và hiện transport qua passthrough `viewPolicy.mobile.bulk`; resolver đọc compatibility shape này sau canonical top-level `bulk`.

Đây là bridge có chủ đích để tránh sửa một brief hàng nghìn dòng cho mỗi grid. **Short-brief compiler/parser first-class transport cho top-level `bulk` vẫn là follow-up hardening**, không được tuyên bố đã hoàn tất trong slice này.
...
7. First-class short-brief compiler/parser transport cho `viewPolicy.bulk`.
