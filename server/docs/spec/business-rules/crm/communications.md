# CRM Communications — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **CRM Communications**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Email/call/WhatsApp records preserve external IDs and threading.
2. Inbound webhook signature and idempotency enforced.
3. Attachment/consent/opt-out policy applies.
4. Send retry never duplicates provider message when provider key exists.

## 3. Lifecycle

- Delivery states are append-only events.
