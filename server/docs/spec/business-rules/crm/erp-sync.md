# CRM ↔ ERP Sync — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **CRM ↔ ERP Sync**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Entity mapping has stable external/source IDs.
2. Conversion to ERP Customer/Quotation/Sales Order is idempotent.
3. Conflict policy is explicit per field: CRM master, ERP master, latest-approved, or manual.
4. Cross-suite operation uses saga/outbox, not distributed transaction.

## 3. Lifecycle

- Compensation never deletes financial document; it marks/reverses through ERP command.
...
- `crm.erp.sync_requested`
- `crm.erp.sync_completed`
- `crm.erp.sync_failed`
