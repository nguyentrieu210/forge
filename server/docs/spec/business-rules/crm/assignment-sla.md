# Assignment, SLA & Automation — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Assignment, SLA & Automation**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Assignment strategies are deterministic for same eligible set; weighted distribution preserves weights.
2. SLA timers respect working calendar, pause states and priority.
3. Automation condition/actions run with loop prevention and execution quota.

## 3. Lifecycle

- Every automatic change records rule version and reason.
