# CRM Deal & Pipeline — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **CRM Deal & Pipeline**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Stage belongs to pipeline and transition policy.
2. Amount, probability, expected close and weighted value remain consistent.
3. Won/lost requires configured reason/details.
4. Drag-drop stage change is one atomic command with audit/activity.

## 3. Lifecycle

- Open stages → Won/Lost; reopen explicit.
