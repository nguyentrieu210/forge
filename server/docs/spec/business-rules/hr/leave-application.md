# Leave Application & Balance — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Leave Application & Balance**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Leave allocation, policy, period, holiday, half-day and carry-forward rules calculate balance.
2. Overlapping approved leave is rejected.
3. Approver/employee visibility and role rules are server-enforced.
4. Cancel restores balance idempotently.
