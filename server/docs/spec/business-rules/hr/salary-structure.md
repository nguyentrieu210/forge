# Salary Structure & Assignment — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Salary Structure & Assignment**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Components have earning/deduction/statistical/tax semantics and formula dependency order.
2. Assignment effective dates cannot produce ambiguous active structures.
3. Formula cycle is publish error.
4. Server recomputes amounts; UI preview is not authority.

## 3. Lifecycle

- Published structure versions are immutable; change creates effective version.
