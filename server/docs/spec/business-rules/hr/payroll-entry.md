# Payroll Entry — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Payroll Entry**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Employee selection respects company, branch, department, designation, payroll frequency and active employment.
2. Salary slips are generated idempotently per employee/period.
3. Submit linked slips then accounting entry/payment per configured workflow.
4. Cancel reverses linked Journal Entry and Payment Ledger effects.

## 3. Lifecycle

- Draft → Slips Created → Submitted/Paid/Cancelled.
