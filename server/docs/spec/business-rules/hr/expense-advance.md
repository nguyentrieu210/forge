# Expense Claim & Employee Advance — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Expense Claim & Employee Advance**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Claim lines require valid category, date, amount, currency and attachment policy.
2. Advance allocation cannot exceed available advance except allowed recovery behavior.
3. Approval status gates accounting/payment.
4. Cancel/reject releases allocations idempotently.

## 3. Lifecycle

- Draft → Approval → Approved/Rejected → Paid/Cancelled.
