# Employee Lifecycle — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Employee Lifecycle**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Employee identity, company, status, dates and user link obey uniqueness/effective-date rules.
2. Transfer/promotion/onboarding/separation create auditable history; no destructive overwrite.
3. Inactive/left employee gates attendance, leave and payroll by posting period.

## 3. Lifecycle

- Applicant/Active/Left/Inactive states are effective-dated.
