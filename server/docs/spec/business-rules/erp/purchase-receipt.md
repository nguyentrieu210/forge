# Purchase Receipt — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Purchase Receipt**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Received/accepted/rejected quantities reconcile.
2. Warehouse, quality inspection, serial/batch and valuation inputs validate.
3. Submit creates SLE and updates purchase order received quantity.
4. Return cannot exceed accepted receipt.

## 3. Lifecycle

- Draft → Submitted → Cancelled; linked return reversal.
