# Delivery Note — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Delivery Note**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Submit validates available stock or configured negative-stock policy.
2. Serial/batch bundle quantities must reconcile item quantity.
3. Updates delivered quantity against Sales Order and creates SLE when stock movement applies.
4. Return reverses quantities against original delivery and cannot over-return.

## 3. Lifecycle

- Draft → Submitted → Cancelled; return is linked reversal document.
- Posting date/time and backdated repost policy apply.
