# Period Close, Repost & Reconciliation — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Period Close, Repost & Reconciliation**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Closed/frozen periods block unauthorized posting.
2. Repost processes deterministic dependency order and is idempotent.
3. Trial balance debit equals credit per company/currency policy.
4. Stock balance reconciles SLE to Bin/current balance and stock GL where perpetual.
5. Period close evidence is immutable and restorable.

## 3. Lifecycle

- Close, reopen and repost require explicit workflow/audit.
