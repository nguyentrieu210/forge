# Sales Invoice — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Sales Invoice**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Taxes/totals, currency conversion, rounding and item-wise tax detail are deterministic.
2. Outstanding = payable total minus advances/allocations/write-off under precision rules.
3. Submit creates GL and optionally SLE when update-stock mode applies.
4. Credit note/return references original and cannot exceed returnable amount.
5. Deferred revenue, loyalty, commission and accounting dimensions preserve configured behavior.

## 3. Lifecycle

- Draft → Submitted → Paid/Partly Paid/Overdue/Return/Cancelled as derived status.
- Cancel posts reversal and reopens linked order/delivery/payment allocations as source behavior dictates.
