# Company & Accounting Basis — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Company & Accounting Basis**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Company currency, fiscal year, chart of accounts and accounting dimensions are mandatory context for ledger-bearing documents.
2. Company cannot be changed after ledger creation without controlled migration.
3. Closed period and frozen-account checks run before ledger plan.

## 3. Lifecycle

- Create company seeds/defaults only through explicit setup transaction.
- Chart templates are versioned imports; no silent overwrite.
