# Serial, Batch & Valuation — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Serial, Batch & Valuation**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Serial number uniqueness and lifecycle enforce one current location/status.
2. Batch expiry and disabled policy gate transactions.
3. Bundle total quantity equals item row quantity.
4. FIFO/moving-average calculation is deterministic and replayable.
5. Negative stock behavior follows item/company policy.

## 3. Lifecycle

- Valuation replay orders by posting timestamp plus stable tie-breaker.
