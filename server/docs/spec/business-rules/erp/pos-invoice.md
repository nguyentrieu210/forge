# POS Invoice & Closing — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **POS Invoice & Closing**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Offline/local cart is provisional until server command succeeds.
2. Payment modes total invoice or create allowed change/credit behavior.
3. Stock, taxes, loyalty and return rules reuse Sales Invoice invariants.
4. Closing entry reconciles opening, invoices and payment modes; discrepancies require reason.

## 3. Lifecycle

- POS retry uses invoice command ID to prevent duplicate sale.
