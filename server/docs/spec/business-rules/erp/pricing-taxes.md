# Pricing, Taxes & Totals — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Pricing, Taxes & Totals**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Pricing rule precedence, promotional scheme, item price and party-specific restrictions are deterministic.
2. UOM conversion occurs before rate/quantity dependent calculations.
3. Tax rows support actual/on-net/on-previous-row/total semantics as source baseline.
4. Inclusive tax, discount and rounding must reproduce golden fixtures.
5. Item-wise tax detail uses relational child rows in v16 baseline.

## 3. Lifecycle

- No UI-only calculation is authoritative; server recomputes.
