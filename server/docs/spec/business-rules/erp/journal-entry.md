# Journal Entry — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Journal Entry**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Total debit equals total credit in company currency after precision rounding.
2. Every line has valid non-group account and party rules where account requires party.
3. Submit posts immutable GL rows; cancel posts reversal, never edits original rows.
4. Inter-company and multi-currency differences follow upstream exchange/rounding behavior.

## 3. Lifecycle

- Draft → Submitted → Cancelled; amend creates new document.
- Period/frozen-account/budget checks before submit.
