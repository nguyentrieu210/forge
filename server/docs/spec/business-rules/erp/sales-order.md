# Sales Order — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Sales Order**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Quotation mapping preserves source references and item conversion factors.
2. Ordered, delivered and billed quantities are tracked independently with tolerance rules.
3. Credit limit and party status gates run before submit where enabled.
4. Close/hold/unhold cannot hide delivered/billed commitments.

## 3. Lifecycle

- Draft → Submitted → On Hold/Closed/Completed/Cancelled.
- Partial delivery and partial billing are first-class.
