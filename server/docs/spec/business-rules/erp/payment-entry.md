# Payment Entry — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Payment Entry**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Party/account/currency rules determine paid-from and paid-to sides.
2. Reference allocation cannot exceed outstanding except explicit overpayment behavior.
3. Exchange gain/loss and deductions balance voucher.
4. Unallocated amount remains party advance according to account rules.

## 3. Lifecycle

- Draft → Submitted → Cancelled.
- Reconciliation changes allocation through audited command.
