# Visual Query, SQL & Python Runtime — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Visual Query, SQL & Python Runtime**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Visual query compiles to typed logical plan before dialect SQL.
2. SQL editor is read-only unless explicit non-ERP sandbox source.
3. Row/byte/time/CPU limits and cancellation are mandatory.
4. Python/Ibis/DuckDB run isolated; output schema/size validated.
5. ERP permission-aware sources apply row/field policy before result.

## 3. Lifecycle

- Same input snapshot + query version yields reproducible result.
