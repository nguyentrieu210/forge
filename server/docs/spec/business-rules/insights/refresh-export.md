# Refresh, Cache & Export — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Refresh, Cache & Export**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Refresh job has source snapshot/bookmark, query version and idempotency key.
2. Cache key includes tenant, permission/policy version, query version, parameters and snapshot.
3. Stale-while-revalidate is allowed only for declared dashboards.
4. Export size threshold routes to Workflow/Container and R2 signed download.

## 3. Lifecycle

- Cache is never authorization source or canonical result history.
