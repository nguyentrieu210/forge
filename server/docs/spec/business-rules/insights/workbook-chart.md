# Workbook & Chart — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Workbook & Chart**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Workbook version owns queries/charts/layout references.
2. Chart config validates field types, aggregation and filter binding.
3. Changing query schema surfaces broken references; never silently drops series.
4. Autosave uses optimistic version.

## 3. Lifecycle

- Draft/published versions; sharing references immutable publish version where configured.
...
- `insights.workbook.saved`
- `insights.chart.rendered`
