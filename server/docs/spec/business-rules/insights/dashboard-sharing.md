# Dashboard, Filters & Sharing — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Dashboard, Filters & Sharing**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Dashboard filter scope is explicit global/chart-specific.
2. Share permission cannot exceed source data permission.
3. Public link is disabled by default and can use expiry/password/domain policy.
4. Export includes as-of snapshot and filter provenance.

## 3. Lifecycle

- Layout conflict uses version merge/reload, not last-write-wins.
...
- `insights.dashboard.published`
- `insights.share.created`
