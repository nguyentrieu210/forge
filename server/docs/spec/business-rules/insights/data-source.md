# Insights Data Source — Business Rule Ledger

## 1. Scope

Contract bắt buộc cho CloudForge parity unit **Insights Data Source**. Exact field/schema/source path được gắn bởi source manifest; ledger này khóa hành vi không được tự suy diễn lại.

## 2. Invariants

1. Credentials are secret references, never returned to client.
2. Connection test and schema sync have timeout/allowlist.
3. Source capabilities declare dialect, read-only, cancellation and incremental cursor.
4. Tenant isolation applies to every query.

## 3. Lifecycle

- Connected/Degraded/Disabled with health evidence.
...
- `insights.source.connected`
- `insights.schema.synced`
