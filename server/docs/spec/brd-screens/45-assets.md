...
- **Tên:** Asset Lifecycle
- **Route:** `/erp/assets`
- **Role:** Asset User, Asset Manager, Accounts Manager
- **Nguồn:** Asset/category/location, capitalization, movement, depreciation, repair, disposal
- **Product pack:** CloudERP

## Khối 2 — Layout
Asset register; lifecycle timeline; depreciation schedule; accounting links.
...
- Submitted movement immutable

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `AssetRegister` | Hiển thị và thao tác Asset Register theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Capitalization` | Hiển thị và thao tác Capitalization theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Movement` | Hiển thị và thao tác Movement theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `DepreciationSchedule` | Hiển thị và thao tác Depreciation Schedule theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Repair` | Hiển thị và thao tác Repair theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Disposal` | Hiển thị và thao tác Disposal theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Capitalize/move/depreciate/repair/sell/scrap | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Adjust value | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Category/accounts/depreciation defaults
...
- Business behavior: `business-rules/erp/asset-lifecycle.md`.
