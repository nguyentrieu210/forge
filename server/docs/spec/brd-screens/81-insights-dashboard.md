# M81 — Insights Dashboard Builder
...
## Khối 1 — Định danh
- **Tên:** Insights Dashboard Builder
- **Route:** `/insights/dashboards/:id`
- **Role:** Analyst, Dashboard Editor
- **Nguồn:** Grid layout, charts, text, global filters, parameters, actions
- **Product pack:** CloudInsights

## Khối 2 — Layout
Responsive canvas; widget palette; global filter wiring; preview/publish.
...
- No broken query/chart publish
- Version rollback

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `DashboardCanvas` | Hiển thị và thao tác Dashboard Canvas theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `GlobalFilterEditor` | Hiển thị và thao tác Global Filter Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ParameterBindings` | Hiển thị và thao tác Parameter Bindings theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `PublishPanel` | Hiển thị và thao tác Publish Panel theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Add/resize/wire/publish/version | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Filter bindings by compatible types
