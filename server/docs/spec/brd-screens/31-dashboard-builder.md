...
## Khối 1 — Định danh
- **Tên:** Dashboard Builder
- **Route:** `/builder/dashboard/:name`
- **Role:** System Manager, Analyst
- **Nguồn:** Dashboard, cards, charts, layout
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
12-column responsive grid; widget palette; source/filter config; preview.
...
- Widget ACL inherited
- No broken source publish

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `GridCanvas` | Hiển thị và thao tác Grid Canvas theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `WidgetPalette` | Hiển thị và thao tác Widget Palette theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `CardEditor` | Hiển thị và thao tác Card Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ChartEditor` | Hiển thị và thao tác Chart Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `FilterEditor` | Hiển thị và thao tác Filter Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
|---|---|---|---|
| Add/resize/configure/publish | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Test query | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Set drilldown | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Source fields/measures suggestions
- Comparison defaults
