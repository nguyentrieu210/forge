# M49 — Commerce, Portal & Subscriptions
...
## Khối 1 — Định danh
- **Tên:** Commerce, Portal & Subscriptions
- **Route:** `/erp/commerce`
- **Role:** Website Manager, Customer/Supplier portal users
- **Nguồn:** Product catalog, cart/order, portal documents, subscription/auto-repeat
- **Product pack:** CloudERP

## Khối 2 — Layout
Catalog/portal; self-service orders/invoices/tickets; subscription schedule.
...
|---|---|---|---|---|
| `ProductCatalog` | Hiển thị và thao tác Product Catalog theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `CartCheckout` | Hiển thị và thao tác Cart Checkout theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `PortalDocuments` | Hiển thị và thao tác Portal Documents theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `SubscriptionManager` | Hiển thị và thao tác Subscription Manager theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `AutoRepeat` | Hiển thị và thao tác Auto Repeat theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `CustomerPortal` | Hiển thị và thao tác Customer Portal theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Browse/order/pay | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Download docs | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Manage subscription | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Customer/price/tax/shipping defaults
