...
## Khối 1 — Định danh
- **Tên:** Backup, Restore & Retention
- **Route:** `/platform/backups`
- **Role:** Platform Operator, Tenant Administrator
- **Nguồn:** D1 backups/exports, R2 snapshots, point-in-time restore policy
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Backup timeline; snapshot detail; restore rehearsal; retention/region policy.
...
|---|---|---|---|---|
| `BackupTimeline` | Hiển thị và thao tác Backup Timeline theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `SnapshotDetail` | Hiển thị và thao tác Snapshot Detail theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `RestoreWizard` | Hiển thị và thao tác Restore Wizard theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `RetentionPolicy` | Hiển thị và thao tác Retention Policy theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `RestoreVerification` | Hiển thị và thao tác Restore Verification theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Create backup | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Restore clone/in-place | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Verify checksum/reconciliation | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Scheduled policy from plan
- Pre-migration snapshot automatic
