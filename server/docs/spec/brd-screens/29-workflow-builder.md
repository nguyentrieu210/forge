...
## Khối 1 — Định danh
- **Tên:** Workflow Builder
- **Route:** `/builder/workflow/:name`
- **Role:** System Manager
- **Nguồn:** Workflow graph model
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Node graph; state/transition editor; impact/orphan analysis.
...
- Full state/transition fields
- Python/server condition equivalent
...
| `GraphCanvas` | Hiển thị và thao tác Graph Canvas theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `StateEditor` | Hiển thị và thao tác State Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `TransitionEditor` | Hiển thị và thao tác Transition Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ImpactPanel` | Hiển thị và thao tác Impact Panel theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Add/connect/reorder/publish | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Test condition | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Migrate documents | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
