...
## Khối 1 — Định danh
- **Tên:** Quality Management
- **Route:** `/erp/quality`
- **Role:** Quality User, Quality Manager
- **Nguồn:** Inspection, procedure, goal, review, feedback, meeting
- **Product pack:** CloudERP

## Khối 2 — Layout
Quality dashboard; inspection plans/results; nonconformance/actions.
...
- Rejected material gates downstream
- Audit result changes

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `QualityInspection` | Hiển thị và thao tác Quality Inspection theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `InspectionTemplate` | Hiển thị và thao tác Inspection Template theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `QualityProcedure` | Hiển thị và thao tác Quality Procedure theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `QualityReview` | Hiển thị và thao tác Quality Review theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `QualityGoal` | Hiển thị và thao tác Quality Goal theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `CorrectiveAction` | Hiển thị và thao tác Corrective Action theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Inspect/accept/reject | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Open corrective action | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Review goal | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Inspection template from item/supplier/customer
