# M56 — Attendance & Employee Check-in
...
## Khối 1 — Định danh
- **Tên:** Attendance & Employee Check-in
- **Route:** `/hr/attendance`
- **Role:** Employee, HR User, HR Manager
- **Nguồn:** Attendance, attendance request, employee checkin, device/geolocation
- **Product pack:** CloudHR

## Khối 2 — Layout
Daily/monthly attendance; exception queue; check-in timeline/map.
...
- Device event idempotency

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `AttendanceGrid` | Hiển thị và thao tác Attendance Grid theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `CheckinTimeline` | Hiển thị và thao tác Checkin Timeline theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `AttendanceRequest` | Hiển thị và thao tác Attendance Request theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `GeoPolicy` | Hiển thị và thao tác Geo Policy theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Check in/out | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Mark/import attendance | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Request correction | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Approve | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Shift/timezone/geolocation rules
