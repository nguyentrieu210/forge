# M17 — Notifications & Communications
...
## Khối 1 — Định danh
- **Tên:** Notifications & Communications
- **Route:** `/platform/communications`
- **Role:** System Manager, Communication Manager
- **Nguồn:** Notification rules, templates, channels, log, opt-out
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Rule builder; template editor; channel settings; delivery log; notification center.
...
|---|---|---|---|---|
| `NotificationRuleBuilder` | Hiển thị và thao tác Notification Rule Builder theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `TemplateEditor` | Hiển thị và thao tác Template Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ChannelSettings` | Hiển thị và thao tác Channel Settings theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `OptOutRegistry` | Hiển thị và thao tác Opt Out Registry theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Create rule | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Send test | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Retry delivery | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Suppress recipient | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Template variables from schema
