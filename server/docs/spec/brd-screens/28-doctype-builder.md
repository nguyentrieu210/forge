...
## Khối 1 — Định danh
- **Tên:** DocType Builder
- **Route:** `/builder/doctype/:name`
- **Role:** System Manager, Developer
- **Nguồn:** Meta schema model + storage compiler
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Sortable tree Tab→Section→Column→Field; palette; property panel; preview.
...
- Standard vs custom mode separated
- Undo/redo/serialization versioned

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `BuilderKernel` | Hiển thị và thao tác Builder Kernel theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `SchemaTree` | Hiển thị và thao tác Schema Tree theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `FieldPalette` | Hiển thị và thao tác Field Palette theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `PropertyPanel` | Hiển thị và thao tác Property Panel theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `LivePreview` | Hiển thị và thao tác Live Preview theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `MigrationPreview` | Hiển thị và thao tác Migration Preview theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Create/reorder/configure fields | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
...
| Customize overlay | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Fieldname suggestion before save
- Control defaults from type
