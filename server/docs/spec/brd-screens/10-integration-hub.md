...
- **Role:** System Manager, Integration Admin
- **Nguồn:** OAuth/API keys, webhooks, email, telephony, payment, social, external DB
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Connector catalog; credential vault; mapping; delivery log; retry/DLQ.
...
|---|---|---|---|---|
| `ConnectorCatalog` | Hiển thị và thao tác Connector Catalog theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `MappingEditor` | Hiển thị và thao tác Mapping Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `WebhookLog` | Hiển thị và thao tác Webhook Log theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `RetryQueue` | Hiển thị và thao tác Retry Queue theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `HealthCheck` | Hiển thị và thao tác Health Check theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Connect/test/disconnect | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Rotate secret | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Replay webhook | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Map fields/status | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Suggested mapping from schemas
- Default retry/idempotency
