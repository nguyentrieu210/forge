...
## Khối 1 — Định danh
- **Tên:** Contacts & Organizations
- **Route:** `/crm/contacts`
- **Role:** Sales User, Sales Manager
- **Nguồn:** Contact, organization, addresses, relationships, linked leads/deals
- **Product pack:** CloudCRM

## Khối 2 — Layout
Directory; 360 profiles; relationship graph; duplicate merge.
...
- PII permission/consent
- Merge reversible evidence

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `ContactDirectory` | Hiển thị và thao tác Contact Directory theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `OrganizationDirectory` | Hiển thị và thao tác Organization Directory theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Profile360` | Hiển thị và thao tác Profile360 theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `RelationshipGraph` | Hiển thị và thao tác Relationship Graph theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `MergeReview` | Hiển thị và thao tác Merge Review theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Create/update/merge/link | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Set primary contact | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Company domain/address suggestions
