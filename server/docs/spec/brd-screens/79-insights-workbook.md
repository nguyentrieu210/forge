# M79 — Workbooks & Query Organization
...
## Khối 1 — Định danh
- **Tên:** Workbooks & Query Organization
- **Route:** `/insights/workbooks/:id`
- **Role:** Analyst, Viewer
- **Nguồn:** Workbook folders, queries, charts, dashboards, notes, versions
- **Product pack:** CloudInsights

## Khối 2 — Layout
Left artifact tree; center editor/viewer; sharing and version history.
...
- Artifact ACL inherited/overridable
- No broken dependency move

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `WorkbookTree` | Hiển thị và thao tác Workbook Tree theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ArtifactTabs` | Hiển thị và thao tác Artifact Tabs theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Notes` | Hiển thị và thao tác Notes theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `SharePanel` | Hiển thị và thao tác Share Panel theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Create/move/duplicate/archive | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Share/publish | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
