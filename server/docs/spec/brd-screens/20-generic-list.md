...
## Khối 1 — Định danh
- **Tên:** Generic List View
- **Route:** `/app/:doctype`
- **Role:** role có read
- **Nguồn:** DocType meta, list settings, filters, indicators
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Three-column desktop list–detail–context; virtualized table; mobile cards.
...
- No unindexed unlimited scans

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `DataGrid` | Hiển thị và thao tác Data Grid theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `FilterBuilder` | Hiển thị và thao tác Filter Builder theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ViewSwitcher` | Hiển thị và thao tác View Switcher theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `BulkToolbar` | Hiển thị và thao tác Bulk Toolbar theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `PreviewPane` | Hiển thị và thao tác Preview Pane theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ContextPane` | Hiển thị và thao tác Context Pane theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Search/filter/sort | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Bulk action/export | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Open/create | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
...
- Columns from meta + user prefs
- Default sort/index aware
