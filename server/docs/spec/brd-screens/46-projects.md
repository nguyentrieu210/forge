...
## Khối 1 — Định danh
- **Tên:** Projects, Tasks & Billing
- **Route:** `/erp/projects`
- **Role:** Project User, Project Manager, Accounts User
- **Nguồn:** Project, task, timesheet, activity cost, billing, profitability
- **Product pack:** CloudERP

## Khối 2 — Layout
Project workspace; task boards/Gantt; time/cost/billing; profitability.
...
- No double billing
- Permission by team/project

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `ProjectDashboard` | Hiển thị và thao tác Project Dashboard theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `TaskBoard` | Hiển thị và thao tác Task Board theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `Timesheet` | Hiển thị và thao tác Timesheet theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Billing` | Hiển thị và thao tác Billing theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Profitability` | Hiển thị và thao tác Profitability theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Plan/assign/update task | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Log/approve time | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Invoice project | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Project template/rates
- Billing from approved time
