...
## Khối 1 — Định danh
- **Tên:** Generic Report View
- **Route:** `/app/:doctype/view/report`
- **Role:** read/report
- **Nguồn:** Meta fields, group/aggregate, saved views
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Column picker, filters, group-by, aggregates, chart toggle, export.
...
- Prepared mode for large result

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `ReportGrid` | Hiển thị và thao tác Report Grid theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `GroupBuilder` | Hiển thị và thao tác Group Builder theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `AggregateBar` | Hiển thị và thao tác Aggregate Bar theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ColumnPicker` | Hiển thị và thao tác Column Picker theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ChartToggle` | Hiển thị và thao tác Chart Toggle theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Run/save/export/schedule | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Drill down | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Suggested numeric aggregates
- Date range defaults
