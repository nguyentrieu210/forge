...
## Khối 1 — Định danh
- **Tên:** Generic Print View
- **Route:** `/print/:doctype/:name`
- **Role:** print permission
- **Nguồn:** Print Format, letterhead, language, document
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Print format chooser; preview; page settings; PDF/email.
...
- Server render trusted templates
...
| `FormatPicker` | Hiển thị và thao tác Format Picker theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `PageSettings` | Hiển thị và thao tác Page Settings theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `LanguagePicker` | Hiển thị và thao tác Language Picker theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `PDFJob` | Hiển thị và thao tác P D F Job theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Print/PDF/email/download | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Choose format | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Default format/letterhead
- Locale/currency
