# M68 — Deal 360, Products & Next Actions
...
## Khối 1 — Định danh
- **Tên:** Deal 360, Products & Next Actions
- **Route:** `/crm/deals/:id`
- **Role:** Sales User, Sales Manager
- **Nguồn:** Deal details, contacts/org, products, activities, forecast, ERP actions
- **Product pack:** CloudCRM

## Khối 2 — Layout
Detail/activity split; product table; competitors; next action; ERP sync.
...
|---|---|---|---|---|
| `DealHeader` | Hiển thị và thao tác Deal Header theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ProductGrid` | Hiển thị và thao tác Product Grid theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `Stakeholders` | Hiển thị và thao tác Stakeholders theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Competitors` | Hiển thị và thao tác Competitors theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ERPActionPanel` | Hiển thị và thao tác E R P Action Panel theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Edit products/forecast | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Communicate | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Create quotation/customer/order | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Products/prices from ERP mapping
