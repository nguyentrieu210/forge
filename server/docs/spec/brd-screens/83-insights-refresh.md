...
## Khối 1 — Định danh
- **Tên:** Refresh, Schedule & Export
- **Route:** `/insights/schedules`
- **Role:** Analyst, Data Source Admin
- **Nguồn:** Source sync, query refresh, extract, dashboard refresh, PDF/CSV/image export
- **Product pack:** CloudInsights

## Khối 2 — Layout
Schedule calendar; job history; dependencies; alerts/notifications.
...
- Idempotent refresh
- Stale/as-of surfaced

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `ScheduleCalendar` | Hiển thị và thao tác Schedule Calendar theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `DependencyDAG` | Hiển thị và thao tác Dependency D A G theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `RefreshJobs` | Hiển thị và thao tác Refresh Jobs theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ExportJobs` | Hiển thị và thao tác Export Jobs theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
|---|---|---|---|
| Schedule/run/cancel/retry | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Set alert/export | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Dependency order automatically calculated
