# M62 — Training, Skills & Employee Wellbeing
...
## Khối 1 — Định danh
- **Tên:** Training, Skills & Employee Wellbeing
- **Route:** `/hr/development`
- **Role:** HR User, Trainer, Employee
- **Nguồn:** Training program/event/result, skill map, health/safety records
- **Product pack:** CloudHR

## Khối 2 — Layout
Training catalog/calendar; enrollment; result/certificate; skills matrix.
...
- Sensitive health data segregated
- Attendance/result audit

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `TrainingCatalog` | Hiển thị và thao tác Training Catalog theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `TrainingEvent` | Hiển thị và thao tác Training Event theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Enrollment` | Hiển thị và thao tác Enrollment theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `TrainingResult` | Hiển thị và thao tác Training Result theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `SkillMatrix` | Hiển thị và thao tác Skill Matrix theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Certificate` | Hiển thị và thao tác Certificate theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Create/enroll/attend/score | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Update skills | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Program/skill recommendations
