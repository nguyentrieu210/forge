# M86 — Cross-Suite Integration & Reconciliation
...
## Khối 1 — Định danh
- **Tên:** Cross-Suite Integration & Reconciliation
- **Route:** `/platform/suite`
- **Role:** System Manager, Auditor, Platform Operator
- **Nguồn:** Versioned ERP–HR–CRM–Insights contracts/events/read models
- **Product pack:** Cross-suite

## Khối 2 — Layout
Contract graph; mappings; reconciliation tabs; mismatch/DLQ queue.
...
- No circular imports
- Every event versioned/idempotent

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `ContractGraph` | Hiển thị và thao tác Contract Graph theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `ReconciliationGrid` | Hiển thị và thao tác Reconciliation Grid theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `MismatchQueue` | Hiển thị và thao tác Mismatch Queue theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ReplayConsole` | Hiển thị và thao tác Replay Console theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `DriftAlert` | Hiển thị và thao tác Drift Alert theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Map/reconcile/replay/rebuild projection | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Mappings from source versions
