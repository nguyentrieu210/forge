...
## Khối 1 — Định danh
- **Tên:** Print Format Builder
- **Route:** `/builder/print/:name`
- **Role:** System Manager, Print Author
- **Nguồn:** Print Format + block model + template runtime
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Paper canvas A4/A5; block palette; data binding; code mode; real-doc preview.
...
- Jinja/template compatibility explicit
- Generated HTML versioned

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `PaperCanvas` | Hiển thị và thao tác Paper Canvas theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `BlockPalette` | Hiển thị và thao tác Block Palette theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `DataBinding` | Hiển thị và thao tác Data Binding theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `CodeEditor` | Hiển thị và thao tác Code Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `Preview` | Hiển thị và thao tác Preview theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Add/reorder/resize | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
...
| Switch block/code mode | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Fields from DocType
- Default page/letterhead
