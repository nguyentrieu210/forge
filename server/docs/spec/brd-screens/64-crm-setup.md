# M64 — CRM Setup & Sales Configuration
...
## Khối 1 — Định danh
- **Tên:** CRM Setup & Sales Configuration
- **Route:** `/crm/settings`
- **Role:** CRM Admin, Sales Manager
- **Nguồn:** Users, teams, territories, stages, fields/layouts, SLA, assignment, integrations
- **Product pack:** CloudCRM

## Khối 2 — Layout
Guided setup; stage/pipeline editor; fields/layout; team/assignment settings.
...
- No stage deletion with active records without migration

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `CRMSetupWizard` | Hiển thị và thao tác C R M Setup Wizard theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `StageEditor` | Hiển thị và thao tác Stage Editor theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `FieldLayoutBuilder` | Hiển thị và thao tác Field Layout Builder theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
...
| `AssignmentRules` | Hiển thị và thao tác Assignment Rules theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `SLASettings` | Hiển thị và thao tác S L A Settings theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Configure/publish | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Import users/data | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Test assignment/SLA | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- ERP company/territory mappings
