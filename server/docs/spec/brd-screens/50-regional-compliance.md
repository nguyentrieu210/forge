# M50 — Regional, Localization & Compliance
...
## Khối 1 — Định danh
- **Tên:** Regional, Localization & Compliance
- **Route:** `/erp/regional`
- **Role:** System Manager, Tax/Compliance Manager
- **Nguồn:** Country charts, tax/e-invoice/e-waybill/local reports and integrations
- **Product pack:** CloudERP

## Khối 2 — Layout
Country pack registry; setup; validation; submission/status/log.
...
- Pack version pinned
- No silent fallback for legal submission

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `CountryPackRegistry` | Hiển thị và thao tác Country Pack Registry theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `LocalizationSetup` | Hiển thị và thao tác Localization Setup theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ComplianceValidator` | Hiển thị và thao tác Compliance Validator theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `SubmissionLog` | Hiển thị và thao tác Submission Log theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `CertificateManager` | Hiển thị và thao tác Certificate Manager theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Install/upgrade pack | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Validate/submit/cancel document | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Retry/report | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |

## Khối 5 — Autofill / Smart defaults
- Defaults from company country/tax IDs
