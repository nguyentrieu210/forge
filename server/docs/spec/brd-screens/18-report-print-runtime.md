...
## Khối 1 — Định danh
- **Tên:** Report & Print Runtime
- **Route:** `/platform/reports`
- **Role:** System Manager, Report Author, Business User
- **Nguồn:** Query/script/report builder, print formats, generated PDF
- **Product pack:** Platform/MetaForge

## Khối 2 — Layout
Report catalog/runner; prepared jobs; print preview; export; cache/lineage.
...
- Large reports async
- Permission applied in query compiler

## Khối 3 — Component
| Component | Hành vi | Nguồn | Quyền | Trạng thái |
|---|---|---|---|---|
| `ReportCatalog` | Hiển thị và thao tác Report Catalog theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ReportRunner` | Hiển thị và thao tác Report Runner theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `PreparedReportJob` | Hiển thị và thao tác Prepared Report Job theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `PrintPreview` | Hiển thị và thao tác Print Preview theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `ExportDialog` | Hiển thị và thao tác Export Dialog theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |
| `LineagePanel` | Hiển thị và thao tác Lineage Panel theo rule ledger và source manifest được dẫn tại “Authoritative contract references” | source contract | server-enforced | loading/empty/error |

## Khối 4 — Hành động
| Thao tác | Validate | Thành công | Lỗi |
|---|---|---|---|
| Run/export/schedule | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Create saved report | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
| Render PDF | quyền + schema + trạng thái + các invariant được dẫn tại “Authoritative contract references” | cập nhật canonical data + audit/outbox | lỗi typed + retry an toàn, không mất draft |
...
- Filters from report definition
- Default print format/letterhead
