...
import { deriveIdentityKey } from "../../../packages/auth/src/index.js";
...
const PROVIDER_ACCOUNT_KEY = "__provider_account__";
...
export async function provisionTenantSecurityV2(
  ...
  const tenantIdentityKey = await deriveIdentityKey(authMaster, tenant, SECURITY_KEY_ID);
  const tenantServiceToken = await deriveInternalServiceTokenV2(serviceMaster, tenant);
  const sessionSecret = randomSecret();

  await putDispatchSecret(providerToken, accountId, namespace, workerName, "INTERNAL_AUTH_SECRET", tenantIdentityKey);
...
async function authorizeProviderRequest(request: Request, env: ControlV2Env, accountId: string): Promise<string> {
  const expectedAccount = (await env.ROUTES.get(PROVIDER_ACCOUNT_KEY))?.trim() ?? "";
