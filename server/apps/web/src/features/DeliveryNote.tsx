...
interface Item { row_id: string; item_code: string; warehouse: string; qty: string; rate: string }
interface Form {
  name: string;
  company: string;
  customer: string;
...
function freshForm(): Form {
  return {
    ...
    posting_date: today(),
    against_sales_order: "",
    items: [{ row_id: "ROW-1", item_code: "ITEM-1", warehouse: "WH-1", qty: "1", rate: "10" }],
  };
}

export function DeliveryNoteScreen({ seed, onHandoff }: { seed: Seed | null; onHandoff: (seed: Seed) => void }) {
  ...
  function buildDocument(): Record<string, unknown> {
    return {
      ...
      against_sales_order: form.against_sales_order,
      items: form.items.map((it) => ({ row_id: it.row_id, item_code: it.item_code, warehouse: it.warehouse, qty: it.qty, rate: it.rate })),
    };
  }
  ...
  async function loadDoc(name: string) {
    await doc.load(async () => {
      const fetched = await api.getDocument<Record<string, unknown>>("Delivery Note", name);
      ...
      setForm({
        ...
        against_sales_order: String(data.against_sales_order ?? ""),
  ...
  // Consume prefill (SO→DN) or open-by-name handoffs.
  useEffect(() => {
    if (!seed || seed.token === consumed.current) return;
    if (seed.kind === "new-delivery") {
      consumed.current = seed.token;
      doc.reset();
      setForm({
        name: newId("DN"),
        company: seed.prefill.company,
        customer: seed.prefill.customer,
        currency: seed.prefill.currency,
        posting_date: today(),
        against_sales_order: seed.prefill.against_sales_order,
        items: seed.prefill.items.map((it, i) => ({
          row_id: `ROW-${i + 1}`,
    ...
    } else if (seed.kind === "open" && seed.doctype === "Delivery Note") {
      consumed.current = seed.token;
  ...
  function addItem() { setForm((f) => ({ ...f, items: [...f.items, { row_id: `ROW-${f.items.length + 1}`, item_code: "", warehouse: "", qty: "1", rate: "0" }] })); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <h2 className="text-lg font-semibold">Delivery Note</h2>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <Field label="Against Sales Order" hint="Submitted Sales Order this delivery draws down">
          <Input value={form.against_sales_order} disabled={!editable} onChange={(e) => patch({ against_sales_order: e.target.value })} placeholder="SO-…" />
        </Field>
      ...
      <div>
        ...
        <Table className="border border-border rounded-md">
          <TableHeader>
            <TableRow>
              <TableHead>Item</TableHead><TableHead>Warehouse</TableHead><TableHead>Qty</TableHead><TableHead>Rate</TableHead><TableHead>Amount</TableHead><TableHead></TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {form.items.map((it, i) => (
              <TableRow key={it.row_id}>
                ...
                <TableCell><Input value={it.warehouse} disabled={!editable} onChange={(e) => patchItem(i, { warehouse: e.target.value })} /></TableCell>
        ...
        <p className="mt-1 text-right text-sm text-muted-foreground">Stock value (preview) <span className="tabular-nums font-medium text-foreground">{total.toFixed(2)}</span></p>
      </div>
      ...
      {doc.isSubmitted && (
        <div className="rounded-md border border-border bg-muted/30 p-3">
          ...
          <p className="mt-2 text-xs text-muted-foreground">Bills the same Sales Order ({form.against_sales_order || "—"}). Set accounts on the next screen.</p>
        </div>
      )}
    </div>
  );
