...
import { assertModifiedMatches, buildCommand, stripServerOwnedFields } from "../../../packages/frappe-api/src/index.js";
import {
...
export async function routeStockReconciliationPreviewApi(
  ...
  await new D1OrganizationSecurityGuard(context.db, metadata).assertMutation(context.tenantId, context.actor, command);
  ...
  const permissions = new MetadataPermissionService(metadata, undefined, new D1DocumentAccessStore(context.db));
