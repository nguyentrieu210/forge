...
export async function routeManufacturingMrpApi(
  ...
  if (prior.length !== readablePrior.length) {
    // Do not reveal whether a hidden plan-generated request exists or what it is named.
...
function rejectTenantSelector(body: JsonObject): void {
  if (Object.hasOwn(body, "tenant_id") || Object.hasOwn(body, "tenantId")) {
    throw errors.validation("MRP tenant scope is controlled by the authenticated server context");
  }
}
