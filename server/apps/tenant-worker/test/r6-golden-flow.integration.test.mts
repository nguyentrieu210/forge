...
describe("R6 Golden Flow on real workerd + D1 + Aggregate DO",()=>{
  it("runs commercial, procurement, manufacture, fulfillment and settlement with authoritative readback",async()=>{
    ...
    expect((await readDoc("Work Order","WO-R6"))?.data.produced_qty).toBe("1.000000");
    ...
    expect((await env.DB.prepare("SELECT COUNT(*) total FROM manufacturing_entries WHERE tenant_id='demo' AND work_order='WO-R6'").first<{total:number}>())?.total).toBeGreaterThan(0);
  });
});
