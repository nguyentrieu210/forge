...
describe("R6 Golden Flow on real workerd + D1 + Aggregate DO",()=>{
  it("runs commercial, procurement, manufacture, fulfillment and settlement with authoritative readback",async()=>{
    ...
    expect((await env.DB.prepare("SELECT COUNT(*) total FROM procurement_entries WHERE tenant_id='demo' AND purchase_order='PO-R6'").first<{total:number}>())?.total).toBeGreaterThan(0);
    const manufacturing=await env.DB.prepare("SELECT COUNT(*) total, COALESCE(SUM(qty_micros),0) qty FROM manufacturing_entries WHERE tenant_id='demo' AND work_order='WO-R6'").first<{total:number;qty:number}>();
    expect(manufacturing?.total).toBeGreaterThan(0);
    expect(manufacturing?.qty).toBeGreaterThan(0);
  });
});
