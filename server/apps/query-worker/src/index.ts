...
import { FinanceStockControlQueryCompiler } from "../../../packages/query/src/finance-stock-control.js";
  ...
  async queue(batch: MessageBatch<PreparedReportMessage>, env: QueryEnv): Promise<void> {
    for (const message of batch.messages) {
      ...
      try {
        ...
        const result = await new D1ReportService(env.DB, new FinanceStockControlQueryCompiler()).run(job.request, true);
        await env.DB.prepare(
