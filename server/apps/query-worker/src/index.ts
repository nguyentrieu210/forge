...
import { FinanceReportCompiler } from "../../../packages/query/src/finance-report-compiler.js";
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/api/v1/reports/run") {
        ...
        if (compiled.prepared) {
          ...
          const jobBookmark = commandBookmark ?? inboundBookmark;
        ...
        // Reports are explicitly non-authoritative. A browser/session bookmark keeps
        // the read at or after the caller's latest committed state; otherwise the
        // nearest D1 replica may serve the report.
        ...
        return jsonResponse(
          result,
          200,
          appendD1ObservationHeaders(new Headers({ "x-cloudforge-trace-id": traceId }), observation),
        );
      }
      ...
      if (request.method === "GET" && match) {
        ...
        // Job state controls retries/user-visible completion. It is not stale-tolerant.
  ...
  async queue(batch: MessageBatch<PreparedReportMessage>, env: QueryEnv): Promise<void> {
    for (const message of batch.messages) {
      ...
      try {
        ...
        const result = await new D1ReportService(reportDb, new FinanceReportCompiler()).run(job.request, true);
        const observation = await observeD1Session(reportDb);
...
async function reconcileStalePreparedReports(env: QueryEnv): Promise<void> {
  ...
  // Reconciliation decides whether to redeliver work, so replica staleness could
  // duplicate execution. Keep this path primary-first.
  const commandDb = openD1Session(env.DB, "authoritative");
  ...
  for (const row of stale.results ?? []) {
    const bookmark = currentD1Bookmark(commandDb);
    await env.REPORT_QUEUE!.send({
