...
async function listSubmittedOrderNames(call: PlatformCall, supplier: string): Promise<string[]> {
  ...
  throw new Error("Có quá nhiều đơn mua; từ chối chọn kỳ đối soát từ dữ liệu bị cắt cụt.");
}
...
async function resolveWindow(call: PlatformCall, supplier: string, queueKey: string, operation: "Close" | "Reverse") {
  ...
  const match = [...candidates.values()].filter((row) => row.status === wanted).sort((a, b) => b.window_sequence - a.window_sequence || b.window_id.localeCompare(a.window_id))[0];
  if (!match) throw new Error(operation === "Close" ? "Quy cách này không có kỳ giao hàng đang mở để đối soát." : "Quy cách này không có kỳ đã đối soát có thể đảo.");
  return match;
}
...
export async function handlePurchaseSupplierSettlement(request: Request, env: PurchaseFifoEnv): Promise<Response> {
  ...
  try {
    ...
    const submittedResponse = await call("method/frappe.client.submit", { method: "POST", body: JSON.stringify({ doc: JSON.stringify(created) }) });
    if (!submittedResponse.ok) throw new Error(`Không ghi sổ được đối soát: ${(await submittedResponse.text()).slice(0, 300)}`);
    const payload = await submittedResponse.json() as { data?: Json; message?: Json }; const submitted = payload.data ?? payload.message ?? created;
    return answer({ ok: true, operation, supplier, queue_key: queueKey, window_id: window.window_id, window_sequence: window.window_sequence, doctype: "Purchase Settlement", name: text(submitted.name ?? created.name), settlement: submitted });
  } catch (error) {
