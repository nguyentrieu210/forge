...
function assertNoDuplicateOrAmbiguousRows(rows: Array<{ item_code?: unknown; batch_no?: unknown }>, source: string): void {
  ...
  for (const [item, state] of batchState) {
    if (state.aggregate && state.batches.size) throw new Error(`${source}: ${item} không được vừa có dòng tổng vừa có dòng theo lô.`);
  }
}
...
function mergeCountLines(draft: ReconciliationDoc, lines: NormalizedCountLine[]): { items: ReconciliationRow[]; extra: NormalizedCountLine[] } {
  ...
  if (missing.length) {
    ...
    throw new Error(`Dữ liệu bulk phải có đủ mọi dòng đã chốt sổ. Còn thiếu ${missing.length} dòng: ${preview}${missing.length > 5 ? ", ..." : ""}.`);
  }
  ...
  for (const [index, line] of extra.entries()) merged.push({ ...line, row_id: `BULK-EXTRA-${index + 1}` });
...
async function previewDraft(call: PlatformCall, draft: ReconciliationDoc, items: ReconciliationRow[]): Promise<PreviewPayload> {
  ...
  const response = await call("v1/inventory/stock-reconciliation/preview", {
    method: "POST",
    body: JSON.stringify({ name: draft.name, modified: draft.modified, document: { ...draft, items } }),
  });
  const payload = await response.json().catch(() => ({})) as PreviewPayload & { message?: unknown; error?: { message?: unknown } };
  if (!response.ok) throw new Error(text(payload.message) || text(payload.error?.message) || `Không preview được phiếu kiểm kê (HTTP ${response.status}).`);
  ...
  if (!payload.document || !Array.isArray(payload.document.items)) throw new Error("Preview kiểm kê không trả về chứng từ chuẩn hóa.");
  return payload;
}
...
export async function handleBulkStockReconciliationRequest(request: Request, env: PurchaseFifoEnv, save: boolean): Promise<Response> {
  try {
    ...
    if (!draft.snapshot_at || !draft.warehouse || !draft.counted_by) throw new Error(`Phiếu ${reconciliationName} chưa có snapshot kiểm kê hợp lệ.`);
    ...
    if (!update.ok) throw new Error(text(updatedPayload.message) || text(updatedPayload.exc) || `Không lưu được ${draft.name} (HTTP ${update.status}).`);
