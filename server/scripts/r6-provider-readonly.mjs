...
if (!/^[0-9a-f]{40}$/i.test(sourceSha)) throw new Error("--source-sha must be the exact 40-character certification candidate SHA");

const evidence = {
  format: "forge-r6-provider-readonly/v1",
  evidence_ids: ["R6-E02", "R6-E05"],
  producer: "R6-CONVERGENCE",
...
let accountIds = [];
if (accountHint) accountIds = [accountHint];
else {
...
if (candidates.length !== 1) {
  block("provider-account-selection", candidates.length === 0 ? "cloudforge-gateway was not readable in any token-visible account" : `cloudforge-gateway resolved in ${candidates.length} accounts; refusing ambiguous evidence`);
  finish();
}
...
checkBindingSet("gateway-bindings", sanitizeBindings(gatewaySettings?.bindings), ["ASSETS", "ROUTES", "DISPATCHER", "CONTROL"]);
...
record("gateway-deployment", gatewayDeployments.ok && deploymentList.length > 0, gatewayDeployments.ok ? { deployment_count_observed: deploymentList.length, latest_deployment_present: deploymentList.length > 0 } : gatewayDeployments.message);
...
record("gateway-observability", gatewayScriptSettings.ok && observabilityEnabled(gatewayScriptSettings.result), gatewayScriptSettings.ok ? sanitizeObservability(gatewayScriptSettings.result) : gatewayScriptSettings.message);
...
record("tenant-worker", tenantDetails.ok, tenantDetails.ok ? sanitizeDispatchDetails(tenantDetails.result) : tenantDetails.message);
...
if (!tenantBindingsResponse.ok) record("tenant-bindings", false, tenantBindingsResponse.message);
else checkBindingSet("tenant-bindings", sanitizeBindings(tenantBindingsResponse.result), ["DB", "FILES", "BROWSER", "AGGREGATES", "OUTBOX_QUEUE", "SOCIAL_INGRESS", "DISPATCHER", "AI"]);
...
record("tenant-observability", tenantSettings.ok && observabilityEnabled(tenantSettings.result), tenantSettings.ok ? sanitizeObservability(tenantSettings.result) : tenantSettings.message);
...
record("alumdoor-app-worker", appDetails.ok, appDetails.ok ? sanitizeDispatchDetails(appDetails.result) : appDetails.message);
...
if (!appBindingsResponse.ok) record("alumdoor-app-bindings", false, appBindingsResponse.message);
else checkBindingSet("alumdoor-app-bindings", sanitizeBindings(appBindingsResponse.result), ["PLATFORM", "AI"]);
...
record("alumdoor-app-observability", appSettings.ok && observabilityEnabled(appSettings.result), appSettings.ok ? sanitizeObservability(appSettings.result) : appSettings.message);

for (const entry of evidence.checks.filter((entry) => entry.ok !== true)) {
  block(entry.name, typeof entry.detail === "string" ? entry.detail : "read-only provider check did not meet acceptance");
}
finish();

function record(name, ok, detail) { evidence.checks.push({ name, ok: Boolean(ok), detail }); }
function block(name, reason) {
  if (!evidence.result.blockers.some((entry) => entry.name === name)) evidence.result.blockers.push({ name, reason });
}
...
function sanitizeBindings(value) {
  return (Array.isArray(value) ? value : []).map((entry) => ({ name: String(entry?.name ?? entry?.binding ?? "").trim(), type: String(entry?.type ?? entry?.kind ?? "unknown").trim() })).filter((entry) => entry.name).sort((a, b) => a.name.localeCompare(b.name));
}
function sanitizeDispatchDetails(value) {
  const script = value?.script ?? value ?? {};
  return { readable: true, compatibility_date: script?.compatibility_date ?? null, compatibility_flags: Array.isArray(script?.compatibility_flags) ? script.compatibility_flags : [] };
}
function observabilityEnabled(value) {
  const candidate = value?.observability ?? value?.settings?.observability ?? null;
  if (!candidate || typeof candidate !== "object") return false;
  return candidate.enabled === true && candidate.logs?.enabled === true && candidate.traces?.enabled === true;
}
function sanitizeObservability(value) {
  const candidate = value?.observability ?? value?.settings?.observability ?? null;
  if (!candidate || typeof candidate !== "object") return { observed: false };
  return { observed: true, enabled: candidate.enabled === true, logs_enabled: candidate.logs?.enabled === true, traces_enabled: candidate.traces?.enabled === true, logpush: typeof value?.logpush === "boolean" ? value.logpush : null };
}
async function api(apiPath, { allowFailure = false } = {}) {
  const response = await fetch(`${baseApi}${apiPath}`, { method: "GET", headers: { authorization: `Bearer ${token}`, accept: "application/json", "user-agent": "forge-r6-provider-readonly/1" } });
  let payload = null;
  try { payload = await response.json(); } catch { payload = null; }
  const ok = response.ok && payload?.success !== false;
  if (!ok && !allowFailure) throw new Error(`Cloudflare read failed ${response.status}: ${firstMessage(payload)}`);
  return { ok, status: response.status, result: payload?.result ?? null, message: firstMessage(payload) || `HTTP ${response.status}` };
}
