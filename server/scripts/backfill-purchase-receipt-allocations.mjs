...
import { findTenantDatabaseId, findTenantOrigin, removeTenantConfig, writeTenantConfig } from "./tenant-wrangler.mjs";
import { d1Query, fail, quote, serverRoot, wrangler } from "./wrangler-cli.mjs";

const args = parseArgs(process.argv.slice(2));
if (!args.tenant) fail("usage: node scripts/backfill-purchase-receipt-allocations.mjs --tenant <id> [--input fixture.json] [--output report.json] [--execute --confirm <id>] [--activate --actor <user> --expected-checksum <sha256>]");
...
try {
  ...
  const report = {
    mode: args.activate ? "activate" : args.execute ? "execute" : "dry-run",
    ...plan,
  };
  const output = args.output
    ? path.resolve(process.cwd(), args.output)
    : path.join(process.cwd(), `purchase-allocation-backfill-${safe(args.tenant)}.json`);
  ...
  if (!source.database) fail("--execute is unavailable with --input; use a remote tenant database");
  if (args.activate) {
...
function readRemote(tenant) {
  ...
  return {
    ...
    progressEntries: d1Query(database, `
      SELECT voucher_type,voucher_no,voucher_revision,line_key,purchase_order,kind,item_code,qty_micros,posting_at
...
function executeBackfill(database, plan, args) {
  ...
  const state = d1Query(database, `
    SELECT enabled,backfill_checksum,unresolved_count
    FROM purchase_allocation_rollout_state
    WHERE tenant_id='${tenant}'`)[0] ?? null;
  if (Number(state?.enabled ?? 0) === 1) fail("purchase allocation rollout is already enabled; backfill cannot be rewritten");
  const existing = d1Query(database, `
    SELECT
      ...
      (SELECT COUNT(*) FROM purchase_unapplied_receipt_entries WHERE tenant_id='${tenant}') AS unapplied`)[0];
  const existingCount = Number(existing?.queues ?? 0) + Number(existing?.obligations ?? 0)
    + Number(existing?.allocations ?? 0) + Number(existing?.unapplied ?? 0);
  if (existingCount > 0) fail("allocation ledger is not empty; use the report to reconcile instead of overwriting history");

  const sql = renderBackfillSql(plan, args.actor ?? "backfill-operator");
  ...
  try {
    ...
    try { process.getBuiltinModule("node:fs").unlinkSync(sqlFile); } catch {}
  }

  const verify = d1Query(database, `
    SELECT
      (SELECT COUNT(*) FROM purchase_obligation_queues WHERE tenant_id='${tenant}') AS queues,
  ...
  if (Number(verify.queues) !== expected.queues
    || Number(verify.windows) !== expected.windows
    || Number(verify.obligations) !== expected.obligations
    || Number(verify.allocations) !== expected.allocations
    || Number(verify.unapplied) !== expected.unapplied
    || String(verify.checksum) !== plan.checksum
    || Number(verify.unresolved) !== 0) {
    fail(`backfill verification failed: ${JSON.stringify(verify)}`);
  }
...
function activate(database, plan, args) {
  if (!/^[a-f0-9]{64}$/.test(args.expectedChecksum)) fail("--expected-checksum must be lowercase SHA-256");
...
function renderBackfillSql(plan, actor) {
  ...
  for (const row of plan.queues) rows.push(`INSERT INTO purchase_obligation_queues(
    ...
    VALUES(${tenant},${sql(row.queue_key)},${sql(row.company)},${sql(row.supplier)},${sql(row.material_match_key)},
      ${row.material_schema_version},${sql(JSON.stringify(row.material_snapshot))},0,${sql(row.created_at)},${sql(row.modified_at)});`);
  for (const row of plan.windows) rows.push(`INSERT INTO purchase_settlement_windows(
    tenant_id,window_id,queue_key,window_sequence,status,tolerance_bps,revision,opened_at)
    VALUES(${tenant},${sql(row.window_id)},${sql(row.queue_key)},${row.window_sequence},'Open',${row.tolerance_bps},0,${sql(row.opened_at)});`);
  for (const row of plan.obligations) rows.push(`INSERT INTO purchase_window_obligation_entries(
    ...
    VALUES(${tenant},${sql(row.entry_id)},${sql(row.queue_key)},${sql(row.window_id)},'Purchase Order',
      ...
      ${sql(row.purchase_order_created_at)},${row.item_idx},${sql(row.committed_at)},${actorSql},${command},'legacy','resolved');`);
  for (const row of plan.allocations) rows.push(`INSERT INTO purchase_receipt_allocation_entries(
    tenant_id,entry_id,queue_key,window_id,voucher_type,voucher_no,voucher_revision,line_key,
    ...
    VALUES(${tenant},${sql(row.entry_id)},${sql(row.queue_key)},${sql(row.window_id)},'Purchase Receipt',
      ...
      ${row.barem_weight_micros},${nullableNumber(row.projected_actual_weight_micros)},${nullableNumber(row.projection_version)},
      ${row.allocation_sequence},${sql(row.posting_at)},${sql(row.committed_at)},${actorSql},'Legacy backfill',
      ${command},'legacy','resolved',NULL);`);
  for (const row of plan.unapplied) rows.push(`INSERT INTO purchase_unapplied_receipt_entries(
    tenant_id,entry_id,queue_key,window_id,voucher_type,voucher_no,voucher_revision,line_key,
    ...
    VALUES(${tenant},${sql(row.entry_id)},${sql(row.queue_key)},${sql(row.window_id)},'Purchase Receipt',
      ${sql(row.voucher_no)},${row.voucher_revision},${sql(row.line_key)},${sql(row.receipt_item_row_id)},
      'receive',${row.qty_micros},${row.barem_weight_micros},${nullableNumber(row.projected_actual_weight_micros)},
      ${nullableNumber(row.projection_version)},NULL,NULL,${sql(row.posting_at)},${sql(row.committed_at)},
      ${actorSql},'Legacy backfill',${command});`);
  rows.push(`INSERT INTO purchase_allocation_rollout_state(
    ...
    ON CONFLICT(tenant_id) DO UPDATE SET
      backfill_checksum=excluded.backfill_checksum,
      unresolved_count=0,
