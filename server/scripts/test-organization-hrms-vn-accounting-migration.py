...
db.executescript((root / "migrations/tenant/0042_hrm_statutory_input_integrity.sql").read_text(encoding="utf-8"))
...
def insert_child(parent_doctype, parent_name, row_id, payload, tenant="demo"):
    db.execute(
        ...
        (tenant, f"{parent_doctype}:{parent_name}", "statutory_inputs", "Payroll Rule Input Value", row_id, 1, json.dumps(payload)),
    )
...
expect_rejected(lambda: insert_child("Salary Structure Assignment", "SSA-INPUT", "R-BAD", {"input_key": "bad", "value": {"nested": 1}}), "HR_STATUTORY_INPUT_INVALID")
...
insert("Employee", "NV-INPUT-SRC", 0, {"company": "ALUMDOOR", "employee_number": "NV003"})
insert("Salary Structure Assignment", "SSA-INPUT-SRC", 1, {"employee": "NV-INPUT-SRC", "company": "ALUMDOOR", "from_date": "2026-11-01", "to_date": "2026-11-30"})
insert_child("Salary Structure Assignment", "SSA-INPUT-SRC", "MOVE", {"input_key": "insured", "value": "1"})
expect_rejected(lambda: db.execute("UPDATE document_children SET parent_key='Salary Structure Assignment:SSA-INPUT' WHERE tenant_id='demo' AND parent_key='Salary Structure Assignment:SSA-INPUT-SRC' AND row_id='MOVE'"), "HR_PAYROLL_SOURCE_LOCKED")
...
valid_rule = {
    ...
    "formula_json": json.dumps({"schema_version": 1, "currency": "VND", "outputs": {"noop": {"const_minor": "0"}}}),
...
doc_rule = {**valid_rule, "rule_code": "VN-DOC", "legal_document_no": "LEGAL-DOC"}
insert("VN Payroll Rule", "VN-DOC", 0, doc_rule)
insert("Salary Structure", "SS-DOC-RULE", 1, {"company": "ALUMDOOR", "payroll_rule": "VN-DOC"})
expect_rejected(lambda: db.execute("UPDATE documents SET payload_json=? WHERE tenant_id='demo' AND doctype='VN Payroll Rule' AND name='VN-DOC'", (json.dumps({**doc_rule, "legal_document_no": "MUTATED-DOC"}),)), "HR_PAYROLL_RULE_IMMUTABLE")
expect_rejected(lambda: db.execute("DELETE FROM documents WHERE tenant_id='demo' AND doctype='VN Payroll Rule' AND name='VN-DOC'"), "HR_PAYROLL_RULE_IMMUTABLE")
expect_rejected(lambda: insert_master("VN Payroll Rule", "VN-DOC", {**valid_rule, "rule_code": "VN-DOC"}), "HR_PAYROLL_RULE_DUPLICATE_STORAGE")
expect_rejected(lambda: insert("VN Payroll Rule", "VN-2026-A", 0, {**valid_rule, "rule_code": "VN-2026-A"}), "HR_PAYROLL_RULE_INVALID")
