#!/usr/bin/env python3
"""Focused SQLite regression for HRM statutory payroll migration 0043."""
...
db.executescript((root / "migrations/tenant/0043_hrm_statutory_payroll_integrity.sql").read_text(encoding="utf-8"))
...
# The freeze is exact-assignment scoped: another overlapping assignment for the same
# employee/company remains mutable until a submitted Salary Slip actually consumes it.
db.execute(
    "UPDATE document_children SET payload_json=? WHERE tenant_id='demo' AND parent_key=? AND row_id=?",
