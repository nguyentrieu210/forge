...
const delayMs = Number(argOf("delay-ms", "2000"));
...
if (!Number.isInteger(attempts) || attempts < 1 || attempts > 30) throw new Error("attempts must be an integer in [1,30]");
if (!Number.isFinite(delayMs) || delayMs < 0 || delayMs > 30_000) throw new Error("delay-ms must be in [0,30000]");
