...
def patch_d1() -> None:
    path = Path("server/packages/document-kernel/src/purchase-allocation-domain-store.ts")
    text = add_imports(path.read_text())
    if "  async getPurchaseSettlementWindowState(" in text:
...
'''
    ...
    path.write_text(head + methods + "\n}" + tail)


def patch_memory() -> None:
    path = Path("server/packages/document-kernel/src/purchase-allocation-in-memory-store.ts")
    text = add_imports(path.read_text())
    if "  async getPurchaseSettlementWindowState(" in text:
        path.write_text(text)
        return
...
'''
    ...
    if anchor not in text:
        raise RuntimeError("snapshot anchor missing")
