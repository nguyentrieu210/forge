...
assert.equal(contract.status, "SYNTHETIC_DRY_RUN_PASS");
...
assert.equal(contract.execution_evidence.workflow_conclusion, "success");
assert.equal(contract.execution_evidence.representative_segments_passed, 9);
assert.equal(contract.execution_evidence.failed_segments, 0);
