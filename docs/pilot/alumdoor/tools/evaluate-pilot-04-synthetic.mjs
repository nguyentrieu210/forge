...
export function evaluatePilot04Synthetic(input, authorities = loadPilot04Authorities()) {
  ...
  gate(
    "NO_P0_P1",
    input?.blockers?.p0 === 0 && input?.blockers?.p1 === 0,
    "no unresolved P0 or P1 blocker may exist",
  );
  gate(
    "ZERO_UNEXPLAINED_VARIANCE",
    input?.reconciliation?.unexplained_variance === 0,
    "unexplained reconciliation variance must be exactly zero",
  );
  gate(
    "RECOVERY_FRESH",
    recoveryFresh(input?.recovery, input?.planned_cutover_at),
    "backup/restore/PITR evidence must be verified and fresh for the rehearsal window",
  );
  gate(
    "CUTOFF_DETERMINISTIC",
    input?.cutoff?.deterministic === true
      && input?.cutoff?.source_bound === true
      && input?.cutoff?.delta_procedure_ready === true,
    "cutoff and delta procedure must be deterministic and source-bound",
  );
  gate(
    "SINGLE_NAMED_DIRECTOR_APPROVAL",
    namedDirectorApproval(input?.approvers),
    "exactly one active named synthetic Giám đốc account must explicitly approve",
  );
