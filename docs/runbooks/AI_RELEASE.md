...
5. Tải artifact, đọc `release.json` và lưu:
   - target SHA;
...
4. GitHub environment `production` phải được approve nếu repository đã cấu hình reviewer.
...
5. Workflow phải chạy đúng thứ tự:
   - backup dry-run;
   - backup live;
   - upload backup;
   - migration dry-run;
