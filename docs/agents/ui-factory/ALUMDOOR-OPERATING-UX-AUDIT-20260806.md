...
`Sidebar phân hệ -> Quy trình -> daily task tabs -> report/history`, with canonical documents and server authority unchanged.

The remaining product surface converges to the same operator model. Daily navigation must answer **what the user needs to do now**, not enumerate every installed DocType/action.
...
| Stock | Alumdoor + canonical stock | navigation sidecar | shared shell / canonical DocTypes/reports/actions | Remove correction/setup noise from daily tabs | declaration first | FAST | new candidate |
| Manufacturing | Alumdoor + canonical manufacturing | navigation sidecar | shared shell / canonical DocTypes/reports/actions | Make order-to-production the entry flow | declaration first | FAST | new candidate |
| Debt | canonical finance | navigation sidecar | shared shell / canonical Payment Entry + reports | Make collection/payable control visible as one module | declaration first | FAST | new candidate |
...
`alumdoor-v2.navigation.json` can override only DocTypes/actions/reports/experiences owned by the Alumdoor brief. It cannot regroup dependency navigation owned by HRM. Extending the shared navigation metadata contract would turn a contained UI task into a STANDARD cross-platform contract change.
...
`Quy trình -> Tồn kho -> Nhập / xuất / chuyển -> Chọn lô cắt -> Cắt nhôm -> Chốt sổ kiểm kê -> Phiếu kiểm kê -> Duyệt kiểm kê -> Lịch sử kho`
...
`Quy trình -> Lập sản xuất -> Lệnh sản xuất -> Năng lực & tăng ca -> Báo cáo -> Lịch sử sản xuất`

`Bill of Materials` and `Production Standard` remain installed setup/master data and leave the daily strip.

### Công nợ

`Quy trình -> Thu / chi công nợ -> Công nợ khách -> Phải thu -> Phải trả`

No new finance logic is created. `Payment Entry`, AR/AP and debt reports remain canonical authority.
...
4. **Reports stay near the decision.** Stock/manufacturing/debt operational reports may be tabs in the owning workspace; global Reports remains available for cross-functional analysis.
