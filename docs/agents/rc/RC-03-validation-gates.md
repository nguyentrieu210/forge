...
- `UI_PROMOTION` + `touches.ui=true` => browser E2E;
- `UI_PROMOTION` + `touches.mobile=true` => mobile E2E.
...
- UI promotion => browser/mobile evidence according to touched surface.
...
- UI promotion => browser/mobile evidence;
...
Result on the exact authored blobs before upload:
...
5. UI promotion adds desktop + mobile evidence when marked applicable.
...
Full Forge typecheck/build/test/E2E was **not run in this connector environment**. That is not reported as PASS and is not needed to invent correctness for the pure policy script.
...
| UI E2E existed but was not promotion-bound | current Playwright config already has desktop + 2 mobile projects | promotion claim now adds browser/mobile gates |
...
## 10. Concurrent-main note

The branch was created from exact `main@3cd2b472068838d0b2b65aa098bbd0bc1a9a8830` as requested. During implementation, `main` advanced by one unrelated UI-only commit touching only `client/packages/ui/src/styles.css`.

Before PR opening, current-main divergence must be rechecked. Since the new main change is disjoint from RC-003 files, it is not a validation-contract dependency and does not invalidate the audit baseline.
