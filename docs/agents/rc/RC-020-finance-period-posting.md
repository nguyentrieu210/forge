...
Status: **PR-ready implementation; RC promotion pending mandatory exact-head build/typecheck/test evidence**  
Risk: **CRITICAL**  
...
2. Canonical accounting controllers remain authoritative for constructing GL lines. `JournalEntryController` builds fixed-point GL and calls `reverseGl(lines)` on cancel.
...
4. `D1MutationStore` persists the canonical document before GL statements and commits document state, `versions`, `gl_entries`, outbox effects and `mutation_receipts` in one D1 batch.
5. Accounting-period enforcement is fail-closed at the tenant DB layer:
   - document triggers fail early for the known posting doctypes inherited from `0042`;
   - the canonical `gl_entries` `BEFORE INSERT` trigger is the universal authority backstop, so any current/future controller emitting GL must pass the same tenant/company/branch period contract even if its doctype is not on the early-validation list.
6. Soft Closed authority is framework-owned `modified_by` plus same-tenant `user_roles`, not client-provided `adjustment_approved_by` metadata.
7. General Ledger / Trial Balance company and branch scope is derived directly from `gl_entries` plus the canonical source `documents` row. `LEFT JOIN` is deliberate so historical orphaned GL is not silently omitted; exceptions are exposed separately.
...
- Soft Closed rejects forged client `adjustment_approved_by` metadata;
- Soft Closed requires period `allow_approved_adjustments`, explicit adjustment intent, non-empty reason and current authenticated close authority (`Chief Accountant`, `Accounts Manager` or `System Manager`), while normal kernel permission remains independently authoritative;
- cancel of a Soft Closed adjustment requires current close authority;
- canonical GL period enforcement covers a GL-producing doctype not present in `0042`'s document whitelist (`Credit Note` regression), proving the period contract is not bypassed by controller naming;
...
- every new GL line must resolve to a same-tenant source company; an explicit source/GL branch mismatch is rejected;
...
- enforces Hard Locked and Soft Closed again at the canonical GL insertion boundary, covering GL producers outside the document-trigger whitelist;
- derives branch scope from source document first and GL dimensions only as branch fallback;
...
- `server/scripts/test-rc020-finance-period-posting.py`
  ...
  - canonical GL period backstop for `Credit Note`, a GL producer outside `0042`'s document whitelist;
  - GL source-scope and branch-mismatch failure paths;
  ...
  - document-before-GL statement ordering source check for the D1 batch.
- `server/tests/rc020-accounting-query-scope.test.mjs`
...
In-session isolated SQLite smoke after the final canonical-GL period-backstop change passed for:

- generic non-whitelisted `Credit Note` GL posting rejected by Hard Locked;
- forged/non-authoritative Soft Closed GL adjustment rejected;
- valid Chief Accountant Soft Closed adjustment accepted at the GL boundary;
- `GL_COMPANY_SCOPE_REQUIRED` failure;
- `GL_BRANCH_SCOPE_MISMATCH` failure.

Earlier isolated SQL smoke for the unchanged report/reconciliation portion passed for:

- company/branch General Ledger separation;
- balanced voucher reconciliation (`difference_minor = 0`);
- zero integrity exceptions for valid rows.

At the time this evidence file was updated, GitHub exposed no workflow runs/statuses for the PR head. This is **not** treated as a task blocker, but it is also not fabricated into green build/typecheck evidence. Mandatory exact-head build/typecheck/unit/integration evidence remains required before RC promotion.
...
- Hard Locked rejects reversal insertion through the canonical GL backstop; Soft Closed allows only adjustment-marked source documents whose current authenticated modifier has close authority.
...
- Soft Closed adds a second DB-layer authority check against `user_roles` for current authenticated/framework-owned `modified_by`.
- Client `adjustment_approved_by` may remain descriptive metadata but is no longer authorization evidence.
- The GL-boundary trigger reads the source document after the store has attributed its mutation to the authenticated command actor, so a new GL controller cannot bypass Soft Closed by supplying a different approver field.
...
- **Blocked part:** branch-aware closed-period enforcement for allocation-only payment-ledger mutations. `Payment Allocation` currently has company/posting scope but no canonical branch field in exact-current metadata and does not emit canonical GL, so the GL backstop cannot govern it indirectly.
- **RC-020 action:** primary canonical-GL posting-period contract is complete independently; no change was made to RC-022's settlement authority.
...
- Mandatory exact-head branch build/typecheck/unit/integration evidence must pass before changing the listed capabilities to **RC**.
...
- PR **#443** is open and not merged.
- GitHub reports the PR mergeable after its mergeability calculation completed.
