...
- `W01-021 Landed cost` and remaining master/UOM/item capabilities retain existing core/procurement evidence and require owner-specific audit; WS04 does not fork procurement logic.
...
- `W02-003 Putaway rule`: **Foundation** `planPutaway` deterministically allocates to caller-approved leaf candidates by priority/capacity and returns explicit unallocated quantity.
...
- `W02-013 Warehouse task assignment`: **Missing first-class persisted task/workflow**; do not invent status model inside React or a vertical generator.
...
Two attempts to create a separate `wms-putaway.test.mjs` were blocked by tool safety classification; equivalent putaway tests were successfully added to `inventory-planning.test.mjs`. No evidence was lost.
