...
- Clean-transplant implementation commit `0901a4f7a9d14083e175271b023f7a63c8725fc9` was created directly from exact `main@1db9c8637e516a57e2a40ffa905af25081a4f8ef`; prior Website delivery branches/PRs are stale iterations and must not merge.
...
- Responsive invariant: public navigation must remain usable on mobile. A prior E2E found desktop/tablet PASS but mobile nav hidden below `md`; `0901a4f7` includes a mobile navigation row with horizontal overflow and `aria-current` rather than weakening the test.
- Quality tier: **CRITICAL** because public unauthenticated routing + tenant isolation are involved. Final delivery requires exact-head server tests/typecheck/build/lint/public E2E + required CI under current workflow definitions.
