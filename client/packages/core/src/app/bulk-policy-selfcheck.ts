...
assert.equal(transaction.enabled, false);
...
const compatibility: DocTypeMeta = {
  ...
  viewPolicy: {
    ...
    mobile: { bulk: { enabled: true, columns: ["item_code", "rate"], editableFields: ["rate"], commitStrategy: "document_update" } },
  },
};
assert.equal(resolveBulkRenderPolicy(compatibility).enabled, true);
