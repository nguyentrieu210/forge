  ...
  return (
    <div className="mf-print-container flex h-full flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-muted/30 p-4">
        {printQ.error ? (
          <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">{adapter.mapError(printQ.error).message}</div>
        ) : (
          <PrintView html={printQ.data ?? ""} title={`${doctype} ${name}`} loading={printQ.isLoading} ref={iframeRef} />
        )}
      </div>
    </div>
  );
