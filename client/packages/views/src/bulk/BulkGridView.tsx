      ...
      <div className="flex flex-wrap items-center gap-2 border-b px-3 py-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline">Nhập hàng loạt</Badge>
          </div>
        ...
        {selectedCount > 0 && policy.allowFillDown && editableNames.length ? (
          <Select onValueChange={(fieldname) => props.onFillDown(fieldname)} disabled={!props.writable}>
            ...
            <SelectContent>
              {editableNames.map((fieldname) => {
                const field = policy.columns.find((candidate) => candidate.fieldname === fieldname);
                return <SelectItem key={fieldname} value={fieldname}>{field?.label ?? fieldname}</SelectItem>;
              })}
            </SelectContent>
          </Select>
        ) : null}
        <Button variant="outline" size="sm" className="h-8" onClick={props.onDiscard} disabled={!dirtyCount || props.saving}>
          <RotateCcw /> Bỏ thay đổi
        </Button>
        <Button size="sm" className="h-8" onClick={props.onSave} disabled={!props.writable || !dirtyCount || props.saving}>
          {props.saving ? <Save className="animate-pulse" /> : <Check />} Lưu {dirtyCount ? `(${dirtyCount})` : ""}
        </Button>
      </div>

      {policy.allowPaste ? (
        <div className="flex items-center gap-1.5 border-b bg-muted/30 px-3 py-1.5 text-xs text-muted-foreground">
          <ClipboardPaste className="size-3.5" /> Có thể dán vùng ô từ Excel/Google Sheets; chỉ các cột được metadata cho phép mới nhận giá trị.
        </div>
      ) : null}
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <Table className="min-w-max">
          <TableHeader className="sticky top-0 z-10 bg-card">
            <TableRow>
              <TableHead className="w-11 min-w-11 sticky left-0 z-20 bg-card">
                <Checkbox checked={allSelected} onCheckedChange={(value) => props.onSelectAll(value === true)} aria-label="Chọn tất cả" />
              </TableHead>
              <TableHead className="w-14 min-w-14">STT</TableHead>
              {policy.columns.map((field) => (
                <TableHead key={field.fieldname} className="min-w-36 whitespace-nowrap">
                  <span>{field.label ?? field.fieldname}</span>
                  {policy.editable.has(field.fieldname) ? <span className="ml-1 text-primary">•</span> : null}
                </TableHead>
              ))}
          ...
          <TableBody>
            {rows.map((row, rowIndex) => {
              ...
              const patch = props.dirty[name];
              return (
                <TableRow key={name} data-state={props.selected.has(name) ? "selected" : undefined}>
                  <TableCell className="sticky left-0 z-10 bg-inherit">
                    <Checkbox checked={props.selected.has(name)} onCheckedChange={(value) => props.onSelect(name, value === true)} aria-label={`Chọn ${name}`} />
                  </TableCell>
                  <TableCell className="tabular-nums text-muted-foreground">{rowIndex + 1}</TableCell>
                  {policy.columns.map((field, columnIndex) => {
                    const value = displayValue(row, patch, field.fieldname);
                    const editable = Boolean(props.writable && policy.editable.has(field.fieldname));
                    return (
                      <TableCell key={field.fieldname} className="p-1.5">
                        {editable ? (
                          <BulkCell
                            field={field}
                            value={value}
                            ...
                            onChange={(next) => props.onCellChange(name, field.fieldname, next)}
                          />
                        ) : <span className="block max-w-72 truncate px-2 text-sm" title={inputValue(value)}>{inputValue(value) || "—"}</span>}
                      </TableCell>
                    );
                  })}
                  <TableCell>
                    {props.errors?.[name]
                      ? <span className="text-xs text-destructive">{props.errors[name]}</span>
                      : patch ? <Badge variant="secondary">Chưa lưu</Badge> : <span className="text-xs text-muted-foreground">Đã đồng bộ</span>}
                  </TableCell>
                </TableRow>
              );
            })}
...
function BulkCell(props: { field: DocField; value: unknown; rowIndex: number; columnIndex: number; onChange: (value: unknown) => void }) {
  ...
  if (props.field.fieldtype === "Check") {
    return (
      <div className="flex h-8 items-center px-2" {...marker} tabIndex={0}>
        <Checkbox checked={Boolean(props.value)} onCheckedChange={(value) => props.onChange(value === true ? 1 : 0)} />
      </div>
    );
  }
  if (props.field.fieldtype === "Select" && optionValues(props.field).length) {
    ...
    return (
      <Select value={value || EMPTY_SELECT} onValueChange={(next) => props.onChange(next === EMPTY_SELECT ? "" : next)}>
        <SelectTrigger className="h-8 min-w-36" {...marker}><SelectValue /></SelectTrigger>
        <SelectContent>
          <SelectItem value={EMPTY_SELECT}>—</SelectItem>
          {optionValues(props.field).map((option) => <SelectItem key={option} value={option}>{props.field.optionLabels?.[option] ?? option}</SelectItem>)}
        </SelectContent>
      </Select>
    );
  }
  const type = NUMERIC_TYPES.has(props.field.fieldtype)
    ? "number"
    : props.field.fieldtype === "Date" ? "date"
      : props.field.fieldtype === "Datetime" ? "datetime-local"
        : "text";
  return (
    <Input
      ...
      onChange={(event) => props.onChange(parseValue(props.field, event.target.value))}
      {...marker}
    />
  );
}
