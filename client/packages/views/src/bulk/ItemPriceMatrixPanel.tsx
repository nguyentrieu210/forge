...
import {
  ...
  Input, Label, ResizableHandle, ResizablePanel, ResizablePanelGroup, Skeleton,
  Table, TableBody, TableCell, TableHead, TableHeader, TableRow, toast,
} from "@metaforge/ui";
import { useDoc, useList } from "../container/hooks.js";
import { useMetaForge } from "../container/provider.js";
import { useBreakpoint } from "../detail/SplitView.js";

export interface ItemPriceMatrixPanelProps {
  adapter: FrappeAdapter;
  onChanged: () => Promise<unknown> | unknown;
  /** Optional current Item when the same price editor is embedded in an app-owned Item detail tab. */
  initialItemCode?: string;
  /** Hide the catalog/tree navigator and keep the editor scoped to initialItemCode. */
  itemLocked?: boolean;
}
...
export function ItemPriceMatrixPanel({ adapter, onChanged, initialItemCode = "", itemLocked = false }: ItemPriceMatrixPanelProps) {
  ...
  const [mobileStep, setMobileStep] = useState<MobileStep>(initialItemCode ? "prices" : "tree");
  ...
  useEffect(() => {
    if (!initialItemCode) return;
    setItemCode(initialItemCode);
    setMobileStep("prices");
  }, [initialItemCode]);
  ...
  const renderGroup = (priceList: string, row: Doc, depth: number): ReactNode => {
    ...
    return <div key={key}>
      <Button type="button" variant="ghost" onClick={() => hasChildren && toggleSet(searching ? setCollapsedSearchGroups : setExpandedGroups, key)} className="h-auto w-full justify-start py-1.5 pr-2 text-left text-sm font-normal" style={{ paddingLeft: `${12 + depth * 16}px` }}>
      ...
      {open ? <div>
        ...
        {directItems.map((item) => <Button key={`${priceList}:${text(item.name)}`} type="button" variant="ghost" onClick={() => selectItem(priceList, item)} className={`h-auto w-full justify-start py-1.5 pr-2 text-left text-sm font-normal ${itemCode === text(item.name) && selectedPriceList === priceList ? "bg-primary/10 font-medium text-primary" : ""}`} style={{ paddingLeft: `${28 + (depth + 1) * 16}px` }}>
    ...
    <div className="min-h-0 flex-1 overflow-auto p-2">
      {visiblePrices.map((price) => {
        ...
        return <div key={name} className="mb-1">
          <Button type="button" variant="ghost" onClick={() => { setSelectedPriceList(name); toggleSet(searching ? setCollapsedSearchPrices : setExpandedPrices, name); }} className={`h-auto w-full items-start justify-start px-2 py-2 text-left font-normal ${selectedPriceList === name ? "bg-primary/10 text-primary" : ""}`}>
  ...
  const matrixPanel = itemCode ? <div className="flex h-full min-h-0 flex-col">
    <div className="flex shrink-0 flex-wrap items-start gap-3 border-b p-4"><div><h3 className="font-semibold">{text(itemDoc?.item_name || itemCode)}</h3><p className="text-xs text-muted-foreground">{itemCode} · ĐVT tồn kho: {stockUom || "—"}</p></div><div className="ml-auto flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setColumnsOpen(true)}><Columns3 /> Cột ({shownPrices.length}/{prices.length})</Button>{!itemLocked ? <Button size="sm" variant="outline" onClick={() => setFocusMode((value) => !value)}>{focusMode ? <Minimize2 /> : <Maximize2 />} {focusMode ? "Thu nhỏ" : "Phóng to"}</Button> : null}<Button size="sm" variant="outline" onClick={() => setAddingUom(true)} disabled={!itemDoc || addingUom}><Settings2 /> Thêm ĐVT</Button><Button size="sm" onClick={() => void save()} disabled={saving || !itemDoc}>{saving ? <RefreshCw className="animate-spin" /> : <Check />} Lưu thay đổi</Button></div></div>
    <div className="min-h-0 flex-1 overflow-auto"><Table unwrapped className="w-max min-w-full border-collapse text-sm"><TableHeader className="sticky top-0 z-10 bg-muted/95"><TableRow><TableHead className="sticky left-0 z-20 min-w-40 border-b border-r bg-muted px-3 py-2 text-left">ĐVT</TableHead><TableHead className="sticky left-40 z-20 min-w-52 border-b border-r bg-muted px-3 py-2 text-left">Quy đổi</TableHead>{shownPrices.map((price) => <TableHead key={text(price.name)} className={`min-w-48 border-b border-r px-3 py-2 text-left ${selectedPriceList === text(price.name) ? "bg-primary/10" : ""}`}><a className="font-medium text-primary hover:underline" href={`/app/Price%20List/${encodeURIComponent(text(price.name))}`}>{text(price.price_list_name || price.name)}</a><span className="mt-0.5 block text-xs font-normal text-muted-foreground"><CalendarDays className="mr-1 inline size-3" />{text(price.effective_date) || "Chưa đặt ngày"}{Number(price.disabled) ? " · Ngừng dùng" : ""}</span></TableHead>)}<TableHead className="w-16 border-b px-2 py-2 text-center">Xóa</TableHead></TableRow></TableHeader><TableBody>{addingUom ? <TableRow className="bg-primary/5"><TableCell className="sticky left-0 z-[2] border-b border-r bg-card p-2"><LinkCombobox id="new-item-uom" value="" target="UOM" label="Đơn vị tính" referenceDoctype="Item" compact search={(doctype, query, options) => adapter.searchLink(doctype, query, options)} onChange={addUom} /></TableCell><TableCell className="border-b border-r px-3 py-2 text-xs text-muted-foreground" colSpan={shownPrices.length + 2}>Tìm và chọn ĐVT để thêm dòng, hoặc <Button type="button" variant="link" size="sm" className="h-auto px-0 py-0 text-xs" onClick={() => setAddingUom(false)}>hủy</Button>.</TableCell></TableRow> : null}{configuredUoms.map((uom) => { const uomName = text(uom.name); const isNew = addedUoms.includes(uomName); return <TableRow key={uomName} className={isNew ? "bg-primary/5" : ""}><TableCell className="sticky left-0 z-[2] border-b border-r bg-card px-3 py-3 font-medium"><span className="block min-w-0 truncate">{text(uom.uom_name || uomName)}{uomName === stockUom ? <Badge className="ml-2" variant="secondary">Tồn</Badge> : null}{isNew ? <Badge className="ml-2">Mới</Badge> : null}</span></TableCell><TableCell className="sticky left-40 z-[2] border-b border-r bg-card px-3 py-2"><div className="flex items-center gap-2"><span className="text-xs text-muted-foreground">1 {uomName} =</span><div className="w-28"><NumberControl field={CONVERSION_FIELD} value={uomName === stockUom ? 1 : (conversionDrafts[uomName] ? numberValue(conversionDrafts[uomName]) : null)} readOnly={uomName === stockUom} compact services={services} onChange={(value) => setConversionDrafts((all) => ({ ...all, [uomName]: value === null || value === undefined ? "" : String(value) }))} /></div><span className="text-xs text-muted-foreground">{stockUom}</span></div></TableCell>{shownPrices.map((price) => { const listName = text(price.name); const draft = drafts[priceKey(listName, uomName)] ?? { rate: "", enabled: false }; return <TableCell key={listName} className={`border-b border-r px-3 py-2 ${selectedPriceList === listName ? "bg-primary/5" : ""}`}><div className="flex items-center gap-2"><Checkbox checked={draft.enabled} onCheckedChange={(value) => setPriceDraft(listName, uomName, { enabled: value === true })} aria-label={`Áp dụng ${uomName} cho ${listName}`} /><div className="w-32"><NumberControl field={PRICE_FIELD} value={draft.rate ? numberValue(draft.rate) : null} readOnly={!draft.enabled || Number(price.disabled) === 1} compact services={services} onChange={(value) => setPriceDraft(listName, uomName, { rate: value === null || value === undefined ? "" : String(value) })} /></div><span className="text-xs text-muted-foreground">đ</span></div></TableCell>; })}<TableCell className="border-b px-2 py-2 text-center">{uomName !== stockUom ? <Button type="button" variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-destructive" onClick={() => removeUom(uomName)} aria-label={`Xóa dòng ${uomName}`}><X /></Button> : null}</TableCell></TableRow>; })}</TableBody></Table>{!shownPrices.length ? <div className="grid h-40 place-items-center text-sm text-muted-foreground">Đang ẩn toàn bộ cột Bảng giá. Bấm Cột để hiện lại.</div> : null}</div>
