...
export function DoctypeWorkspace(props: DoctypeWorkspaceProps) {
  ...
  const modeTabs = bulkEnabled && !decoded && !isNew ? (
    <div className="flex shrink-0 items-center gap-1 border-b bg-card px-3 py-2">
      <Button variant={bulkActive ? "ghost" : "secondary"} size="sm" className="h-8" onClick={() => bridge.set({ view: null })}>
        <List /> Danh sách
      </Button>
      <Button variant={bulkActive ? "secondary" : "ghost"} size="sm" className="h-8" onClick={() => bridge.set({ view: "bulk" })}>
  ...
  return (
    <>
          ...
          {bulkActive ? (
            ...
            <SplitView
              ...
              detail={decoded ? (
                ...
                />
              ) : isTree ? <div className="grid h-full place-items-center bg-card px-6 text-center text-sm text-muted-foreground">{t("common.choose_prefix")} {displayTitle.toLocaleLowerCase("vi")}</div> : null}
              context={decoded ? (
                <ContextContainer
      ...
      <Dialog open={isNew} onOpenChange={(open) => { if (!open) setCloseRequest((value) => value + 1); }}>
        <DialogContent className="flex h-[min(92vh,900px)] w-[min(96vw,760px)] max-w-none flex-col overflow-hidden p-0" onInteractOutside={(event) => { event.preventDefault(); setCloseRequest((value) => value + 1); }} onEscapeKeyDown={(event) => { event.preventDefault(); setCloseRequest((value) => value + 1); }}>
          <DialogHeader className="shrink-0 border-b px-5 py-3"><DialogTitle>{t("form.create_title_prefix")} {displayTitle.toLocaleLowerCase("vi")}</DialogTitle></DialogHeader>
          <div className="min-h-0 flex-1 overflow-hidden">
            <NewFormContainer doctype={doctype} closeRequest={closeRequest} onCreated={(newName) => onNavigate(`${listPath}/${encodeURIComponent(newName)}`)} onCancel={() => onNavigate(listPath)} />
          </div>
        </DialogContent>
      </Dialog>
    </>
