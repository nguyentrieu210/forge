...
/**
 * Prefer first-class `AppAction.input_tables` while preserving rolling compatibility.
 *
 * The current stable ActionScreen grid renderer still accepts the historical Text-field
 * transport. This adapter converts only at the presentation boundary, removes any matching
 * fallback field first, and never mutates the server-provided action. Therefore:
 *
 * - new clients consume `input_tables` as the source of truth;
 * - old installed packages without `input_tables` continue unchanged;
 * - server permission/business validation remains authoritative;
 * - the compatibility conversion can be deleted when ActionScreen renders tables directly.
 */
