...
export function AlumdoorSalesSheetV3() {
  ...
  const renderDiscountCell = (line: SaleLine, index: number, column: ColumnDef) => {
    if (column.key === "item") return <div className="px-1.5 text-xs font-semibold">Chiết khấu</div>;
  ...
  return <div className="h-full w-full overflow-auto bg-white p-2 md:p-3">
    <div className="mx-auto w-full max-w-[1900px] space-y-2">
      <section className="grid border-l border-t border-slate-300 bg-white md:grid-cols-4">
        ...
        {customer.name ? <div className="border-b border-r border-slate-300 p-2 text-[11px] text-slate-600 md:col-span-4">{[customer.phone, priceList ? `Bảng giá: ${priceList}` : "", currency, createdOrder?.name ? String(createdOrder.name) : ""].filter(Boolean).join(" · ")}{submitted ? <span className="ml-3 inline-flex items-center gap-1 font-semibold text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5" />Đã xác nhận</span> : null}</div> : null}
      </section>

      <div className="flex items-center gap-2">
        {!submitted ? <Button type="button" variant="outline" size="sm" onClick={() => setLines((current) => [...current, newLine()])}><Plus /> Dòng</Button> : null}
        <span className="ml-auto text-[11px] text-slate-500">Ô viền đỏ = cần nhập · ô không áp dụng để trắng · giá, diện tích, ĐVT và thành tiền tự điền</span>
      </div>

      <div className="w-full overflow-x-auto border border-slate-300 bg-white">
        <table className="w-max min-w-full border-collapse text-xs">
          <thead><tr><th className="sticky left-0 z-20 w-10 min-w-10 border border-slate-300 bg-slate-50 px-1 py-2 text-center font-semibold">STT</th>{COLUMNS.map((column) => <th key={column.key} className="border border-slate-300 bg-slate-50 px-1 py-2 text-center font-semibold" style={{ width: `${column.width}rem`, minWidth: `${column.width}rem` } as CSSProperties}>{column.label}</th>)}{!submitted ? <th className="w-10 min-w-10 border border-slate-300 bg-slate-50"></th> : null}</tr></thead>
      ...
      <section className="ml-auto w-full max-w-md border-l border-t border-slate-300 bg-white">
        ...
        <div className="grid grid-cols-2"><div className="border-b border-r border-slate-300 p-2 text-sm">Giao nhận</div><div className="border-b border-r border-slate-300"></div></div>
        <div className="grid grid-cols-2"><div className="border-b border-r border-slate-300 p-2 text-sm">Thuế</div><div className="border-b border-r border-slate-300"></div></div>
        <div className="grid grid-cols-2"><div className="border-b border-r border-slate-300 p-2 text-sm">Đã thu</div><div className="border-b border-r border-slate-300"></div></div>
        <div className="grid grid-cols-2"><div className="border-b border-r border-slate-300 p-2 text-sm font-bold">CÒN PHẢI THU</div><div className="border-b border-r border-slate-300 p-2 text-right text-sm font-bold tabular-nums">{total ? money(total) : ""}</div></div>
      </section>
...
function FragmentRows({ line, lineIndex, submitted, missing, renderProductCell, renderDiscountCell, onDelete }: {
  ...
  return <>
    ...
    <tr>{COLUMNS.map((column) => <td key={column.key} className={`h-8 border border-slate-300 bg-white p-0 align-middle ${column.numeric ? "text-right" : ""}`}>{renderDiscountCell(line, lineIndex, column)}</td>)}</tr>
