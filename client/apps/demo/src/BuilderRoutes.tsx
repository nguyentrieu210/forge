...
import {
  DocTypeBuilder, MetadataIntelligenceEditor, WorkflowBuilder, PrintFormatBuilder, DashboardBuilder,
    ...
    <header className="overflow-hidden rounded-2xl border bg-card shadow-sm">
      <div className="relative isolate overflow-hidden bg-primary px-4 py-5 text-primary-foreground sm:px-5">
        ...
        <div className="relative flex items-start gap-3">
          <div className="grid size-10 shrink-0 place-items-center rounded-xl border border-primary-foreground/15 bg-primary-foreground/10 shadow-inner">{icon}</div>
...
export default function BuilderRoutes(props: BuilderRoutesProps) {
    ...
    return (
      <div className="space-y-4">
        <StudioHeader
          ...
          description="Thiết kế cấu trúc dữ liệu, khai metadata intelligence và kiểm tra ngay trên form runtime thật."
        ...
        <div className="grid items-start gap-4 2xl:grid-cols-[minmax(0,3fr)_minmax(22rem,2fr)]">
          <div className="space-y-4">
            ...
            />
            <MetadataIntelligenceEditor meta={builtMeta} onChange={setBuiltMeta} />
          </div>
          <aside className="overflow-hidden rounded-2xl border bg-card shadow-sm 2xl:sticky 2xl:top-3">
            ...
            <div className="h-[min(42rem,75vh)] overflow-auto bg-background/60 p-3 sm:p-4">
              <FormView
                key={JSON.stringify(builtMeta.fields.map((field) => [field.fieldname, field.valueSource, field.editMode, field.surface, field.fetch_from, field.link_filters]))}
                meta={builtMeta}
  ...
  if (which === "b-workflow") {
    return (
      <div className="space-y-4">
        <StudioHeader icon={<Workflow className="size-5" />} eyebrow="Meta Studio · Process" title="Workflow Builder" description="Mô hình hóa trạng thái, hành động và quyền chuyển tiếp trên một canvas quy trình duy nhất." meta="Node graph" />
  ...
  if (which === "b-print") {
    return (
      <div className="space-y-4">
        <StudioHeader icon={<Printer className="size-5" />} eyebrow="Meta Studio · Output" title="Print Format Builder" description="Chọn dữ liệu cần in, sắp xếp thứ tự và kiểm tra bố cục giấy trước khi phát hành mẫu." meta="Paper blocks" />
  ...
  if (which === "b-dashboard") {
    return (
      <div className="space-y-4">
        <StudioHeader icon={<LayoutDashboard className="size-5" />} eyebrow="Meta Studio · Analytics" title="Dashboard Builder" description="Ghép chỉ tiêu và biểu đồ trên lưới responsive để tạo cockpit vận hành bằng metadata." meta="Responsive grid" />
